...
<main class="page--inner page--product--detail">
    <div class="container">
        <article class="product--detail">
            <div class="product__header">
                <div class="main-swiper-detail product__thumbnail">
                    <div class="product__thumbnail_detail">
                        @php 
                            $checkTab = false;
                        @endphp
                        @foreach ($productImages as $keyImg => $img)
                            <div class="wrapper box-shadow banner-effect" data-order="{{$keyImg+1}}" style="order: {{$keyImg+1}}">
                                <div class="product__gallery" data-arrow="true">
                                    <div class="item">
                                        <a href="{{ RvMedia::getImageUrl($img) }}">
                                            <img src="{{ RvMedia::getImageUrl($img) }}" alt="{{ $product->nameHomepage }}" loading="lazy" />
                                        </a>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if(!$checkTab)
                                @if(count($productImages) <= 2)
                                    @if($keyImg === count($productImages) - 1)
                                        <div class="product__content tab-root" style="order: 3;">
                                            <ul class="tab-list">
                                                <li><a href="#tab-description">{{ __('Description') }}</a></li>
                ...
                <div class="product__info">
                    <div class="product__sticky">
                        <div class="product__info-header">
                            <h2 class="product__title">{!! BaseHelper::clean($product->nameHomepage) !!}</h2>
                        ...
                        <div class="product__shopping">
                            @if (EcommerceHelper::isCartEnabled())
                                <form class="single-variation-wrap add-to-cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}">
                                    ...
                                    <button type="button" class="btn_chart-full" data-toggle="modal" data-target="#modalSizeChart">
                                        <i class="feather icon icon-bar-chart-2"></i> {{ __('Size Chart') }}
                                    </button>
                        ...
                        <div class="product__content tab-root">
                            <ul class="tab-list">
                                <li><a href="#tab-description-sm">{{ __('Description') }}</a></li>
                                @if (EcommerceHelper::isReviewEnabled())
                                    <li><a href="#tab-reviews-sm">{{ __('Reviews') }}({{ $product->reviews_count }})</a></li>
                                @endif
                                @if (is_plugin_active('faq'))
                                    @if (count($product->faq_items) > 0)
                                        <li>
                                            <a href="#tab-faq-sm">{{ __('Questions and Answers') }}</a>
                                        </li>
                                    @endif
                                @endif
                            </ul>
                            <div class="tabs">
                                <div class="tab" id="tab-description-sm">
                                    <div class="document ck-content">
                                        {!! BaseHelper::clean($product->content) !!}
                                    </div>
                                </div>
                                @if (is_plugin_active('faq') && count($product->faq_items) > 0)
                                    <div class="tab faqs-list" id="tab-faq-sm">
                                        <div class="accordion" id="faq-accordion">
                                            @foreach($product->faq_items as $faq)
                                                <div class="card">
                                                    <div class="card-header" id="heading-faq-{{ $loop->index }}">
                                ...
                                @endif
                                @if (EcommerceHelper::isReviewEnabled())
                                    <div class="tab product-reviews-container" id="tab-reviews-sm">
                                        @include('plugins/ecommerce::themes.includes.reviews')
                                    </div>
                                @endif
                            </div>
        ...
        @php
            $crossSellProducts = get_cross_sale_products_homepage($product);
        @endphp
        ...
        <section class="section--related-posts">
            ...
            @php
                $relatedProducts = get_related_products_homepage($product, 6);
            @endphp
            <div class="row">
    ...
    @include(Theme::getThemeNamespace() . '::views/ecommerce/includes/modal-sizechart')
</main>
