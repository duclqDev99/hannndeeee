...
class ProductionTable extends TableAbstract
{
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('order_code', function (OrderProduction $item) {
                ...
                return Html::link(route('retail.sale.purchase-order.index') . '?order_code=' . $paramCode, $item->order_code);
            })
            ...
            ->editColumn('expected_date', function (OrderProduction $item) {
                return Carbon::parse($item->order->expected_date)->format('d-m-Y');
            })
            ...
            ->editColumn('created_by', function (OrderProduction $item) {
                $created = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_CREATE_PRODUCTION, $item->order->id);
                if ($created) {
            ...
            ->editColumn('operators', function (OrderProduction $item) {
                return view('plugins/order-retail::sale.production.dropdown-actions', compact('item'));
            });
    ...
    {
        return $this->addCreateButton(route('retail.sale.production.create'), 'retail.sale.production.create');
    }
