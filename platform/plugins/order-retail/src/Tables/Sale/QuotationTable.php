...
class QuotationTable     extends TableAbstract
{
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('customer_phone', function (OrderQuotation $item) {
                return $item->order->customer_phone;
            })
            ->editColumn('customer_confirm', function (OrderQuotation $item) {
            ...
            ->editColumn('contract', function (OrderQuotation $item) {
                if ($contract = $item->contract) {
                    $url = RvMedia::getImageUrl($contract->url);
                    return '<a href="' . $url . '" download>' . $contract->extras['filename'] . '</a>';
                }
                return '';
            })
            ...
            ->editColumn('invoice', function (OrderQuotation $item) {
                $route = route('retail.sale.quotation.get-invoice', $item->id) . '?type=print';
                return '<a class="btn" href="' . $route . '"><i class="fa-solid fa-download"></i></a>';
            })
            ->editColumn('status', function (OrderQuotation $item) {
                return '<span class="text-wrap" style="max-width:100px">' . $item->order->lastAction->actionSetting->title . '</span>';
            })
            ->editColumn('operators', function (OrderQuotation $item) {
                return view('plugins/order-retail::sale.quotation.dropdown-actions', compact('item'));
            });
    ...
    public function query(): Relation|Builder|QueryBuilder
    {
        ...
        $query = $this
            ...
            ->with([
                ...
                'order.lastACtion' => fn ($q) => $q->whereRelation('actionSetting', 'hd_action_setting.action_type', 'quotation_order')
            ])
    ...
    {
        return $this->addCreateButton(route('retail.sale.quotation.create'), 'retail.sale.quotation.create');
    }
