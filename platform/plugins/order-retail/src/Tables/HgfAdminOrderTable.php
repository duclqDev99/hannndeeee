...
class HgfAdminOrderTable extends TableAbstract
{
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('note', function (Order $item) {
                return "---";
            })
            ->editColumn('status', function (Order $item) {
                // if (isset($item->steps[0]->departments[0]->handler)) {
                //     return '<span class="show_detail_status_btn" data-id="' . $item->steps[0]->departments[0]->id . '" role="button" data-bs-toggle="modal" data-bs-target="#statusDetailModal">
                //       ' . $item->steps[0]->departments[0]->handler->first_name . ' ' . $item->steps[0]->departments[0]->handler->last_name
                //         . ' '
                //         . OrderStepStatusEnum::getLabel($item->steps[0]->departments[0]->status) . '
                //     </span>';
                // }
                return "Chờ duyệt";
            })
            ->editColumn('operators', function (Order $item) {
                return view('plugins/sales::hgf.admin.table.columns.dropdown-actions', compact('item'));
            });
