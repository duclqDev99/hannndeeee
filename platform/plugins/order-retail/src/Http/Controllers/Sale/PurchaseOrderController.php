...
use Botble\OrderRetail\Forms\Sale\AddProductForm;
use Botble\OrderRetail\Forms\Sale\PurchaseOrderForm;
...
use Botble\OrderRetail\Tables\Sale\PurchaseOrderTable;
...
use Botble\OrderRetail\Models\ProductSize;
...
class PurchaseOrderController extends BaseController
{
    public function __construct(
        ...
        $this
            ->breadcrumb()
            ->add(trans('Quản lý yêu cầu sản xuất'), route('purchase-order.index'));
    }

    public function index(PurchaseOrderTable $table)
    {
        PageTitle::setTitle("Quản lý yêu cầu sản xuất");
    ...
    public function search(Request $request)
    {
        ...
        $orders = Order::query()
            ->when($type == 'quotation', function ($q) {
                $q->whereHas('steps.actions', function ($q) {
                    $q->where(function ($q) {
                        $q->where('hd_action.action_code', ActionEnum::RETAIL_SALE_MANAGER_CONFIRM_ORDER);
                        $q->whereCompleted();
                    });
                });
                $q->whereHas('steps.actions', function ($q) {
                    $q->where(function ($q) {
                        $q->where('hd_action.action_code', ActionEnum::HGF_ADMIN_CONFIRM_ORDER);
                        $q->whereCompleted();
                    });
                });
                $q->whereDoesntHave('quotation');
            })
            ->when($type == 'production', function ($q) {
                $q->whereHas('steps.actions', function ($q) {
                    $q->where(function ($q) {
                        $q->where('hd_action.action_code', ActionEnum::CUSTOMER_DEPOSIT);
                        $q->whereCompleted();
                    });
                });
    ...
    public function show(Order $order)
    {
        ...
        return view('plugins/order-retail::sale.purchase-order.show', compact('order'));
    }

    public function appendToQuotationForm(Request $request)
    {
        $orderCode = $request->order_code;
        $order = Order::query()->where('code', $orderCode)->first();
        $order->loadMissing([
            'products' => fn($q) => $q->withSum('sizes as qty', 'quantity')
        ]);
        return view('plugins/order-retail::form.append-order-info-to-quotation-form', compact('order'));
    }

    public function appendToProductionForm(Request $request)
    {
        $orderCode = $request->order_code;
        $order = Order::query()->where('code', $orderCode)->first();
        $order->loadMissing([
        ...
        return view('plugins/order-retail::form.append-order-info-to-production-form', compact('order'));
    }
    ...
    public function create(FormBuilder $formBuilder)
    {
        ...
        $form = $formBuilder->create(PurchaseOrderForm::class, ['enctype' => 'multipart/form-data']);
        return view('plugins/order-retail::form.purchase-order-form', compact('form'));
    }

    public function addProductToPurchaseOrderForm(FormBuilder $formBuilder)
    {

        return view('plugins/order-retail::form.add-product');
    }

    public function store(PurchaseOrderRequest $request, StepService $stepService)
    {

        $data =  $request->input();
        $files = $request->file('product_files');
        $requestData = $request->input();

        $products = collect($data['products']);
     

        $productsWithFiles = $products->map(function ($product, $index) use ($files) {
            $product['files'] = $files[$index];
            return $product;
        });

        $data['products'] = $productsWithFiles;

        DB::beginTransaction();
        try {
            $orderData = [];
            Arr::set($orderData, 'order_type', $data['order_type']);
            Arr::set($orderData, 'customer_name', $data['customer_name']);
            Arr::set($orderData, 'customer_phone', $data['customer_phone']);
            Arr::set($orderData, 'expected_date', $data['expected_date']);

            // Arr::set($orderData, 'expected_date', date_format(date_create_from_format('d-m-Y', $data['expected_date']), 'Y-m-d'));
            Arr::set($orderData, 'note', $data['note']);
            Arr::set($orderData, 'created_by_id', Auth::id());
            ...
            foreach ($data['products'] as $product) {
                Arr::set($product, 'order_id', $order->id);
                $files = $product['files'];
                $sizes = $product['sizes'];
                $productTotalQty = 0;
                ...
                if ($fileDesign = Arr::get($files, 'file_design')) {
                    $saveFileResult = $this->saveProductFile($fileDesign, 'product-design-files');
                    if ($saveFileResult) {
                        Arr::set($saveFileResult, 'retail_product_id', $orderProduct->id);
                        ProductDesignFile::create($saveFileResult);
                    }
                }

                if ($fileImages = Arr::get($files, 'images')) {
                    foreach ($fileImages as $fileImage) {
                        $saveFileResult = $this->saveProductFile($fileImage, 'product-image-files');
                        if ($saveFileResult) {
                            Arr::set($saveFileResult, 'retail_product_id', $orderProduct->id);
                            ProductImageFile::create($saveFileResult);
                        }
                    }
                }

                foreach ($sizes as $size) {
                    $productTotalQty += (int) $size['quantity'];
                    ProductSize::create([
                        'value' => $size['value'],
                        'quantity' => $size['quantity'],
                        'retail_product_id' => $orderProduct->id
                    ]);
                }
                $totalQty += $productTotalQty;
                $totalAmount =  $productTotalQty * $product['price'];
            }
            $order->update([
                'amount' => $totalAmount,
                'total_qty' => $totalQty,
            ...
            $stepService->init($order);
            $stepService->updateStep(ActionEnum::RETAIL_SALE_CREATE_ORDER, [
                'order_id' => $order->id,
            ...
            return $this
                ->httpResponse()
                ->setData($order)
                ->setNextUrl(route('retail.sale.purchase-order.index'))
                ->setMessage("Tạo thành công 1 yêu cầu sản xuất");
        } catch (Exception $err) {
            DB::rollBack();
            return $this->httpResponse()->setError()->setMessage($err->getMessage());
        }
    }
    ...
    public function edit(Order $order, FormBuilder $formBuilder)
    {
        ...
        $this->pageTitle('Chỉnh yêu cầu sản xuất ' . $order->code);
        ...
        $order->loadMissing(['products' => fn($q) => $q->withSum('sizes as qty' , 'quantity')]);

        return view('plugins/order-retail::sale.purchase-order.edit', compact('order'));
    }
    ...
    public function update(Order $order, Request $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setPreviousUrl(route('retail.sale.purchase-order.index'))
            ->setNextUrl(route('retail.sale.purchase-order.index'))
