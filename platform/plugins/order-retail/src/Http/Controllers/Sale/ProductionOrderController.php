...
use Botble\Department\Enums\DepartmentCodeEnum;
...
class ProductionOrderController extends BaseController
{
    public function __construct(
        ...
        $this
            ->breadcrumb()
            ->add(trans('plugins/sales::orders.menu'), route('order-production.index'));
    }

    public function index(OrderProductionTable $table)
    {
        dd('asdasd');
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $purchaseOrder = Order::query()->create([
                'general_code' => $requestData['general_code'],
            ...
            $stepInfos = StepInfo::with('details')->get();
            $stepInfos->each(function ($stepInfo, $index) use ($purchaseOrder) {
                $orderStep = Step::create([
                ...
                $stepInfo->details->each(function ($stepDetail) use ($orderStep) {
                    StepDetail::create([
                        'title' => $stepDetail->title,
                        'department_code' => $stepDetail->department_code,
                        'is_show' => $stepDetail->is_show,
                        'step_id' => $orderStep->id,
                        'action_code' => $stepDetail->action_code,
                        'action_type' => $stepDetail->action_type,
                        'update_relate_actions' => $stepDetail->update_relate_actions
                    ]);
                });
            });

            StepDetail::where('action_code', StepActionEnum::RETAIL_SALE_CREATE_ORDER)
                ->whereHas('step.order', fn ($q) => $q->where('hd_orders.id', $purchaseOrder->id))
                ->first()->update([
            ...
            //     'permission' => false,
            //     'route' => route('order-production.edit', $purchaseOrder),
            //     'status' => 'Chờ duyệt'
            // ];
            // $action = 'Người ' . $arrNoti['action'] . ": " . Auth::user()->name . ". Tiêu đề: " . $purchaseOrder->title;

            //Create event for admin
            // event(
            //     new AdminNotificationEvent(
            //         AdminNotificationItem::make()
            //             ->title(ucfirst($arrNoti['action']) . ' đơn yêu cầu sản xuất')
            //             ->description($action)
    ...
    public function edit(Order $orderProduction, FormBuilder $formBuilder)
    {
        Assets::addStylesDirectly(['vendor/core/plugins/ecommerce/css/ecommerce.css'])
            ->addScriptsDirectly([
                'vendor/core/plugins/ecommerce/libraries/jquery.textarea_autosize.js',
        ...
        $order = $orderProduction->load(['order_detail']);

        $this->pageTitle(trans('core/base::forms.edit_item', ['name' => $orderProduction->order_code]));
    ...
    public function update(Order $order, Request $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setPreviousUrl(route('order-production.index'))
