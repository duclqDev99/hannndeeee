@php
    $saleStep = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_REQUESTING_APPROVE_ORDER, $item->id);
    $saleAdminStep = get_action(
        \Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_MANAGER_CONFIRM_ORDER,
        $item->id,
    );
    $hgfAdminStep = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::HGF_ADMIN_CONFIRM_ORDER, $item->id);
@endphp
...
<div class="table-actions">
    <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdown-actions-57e516785256609e1606c1598309234b-120" style="">
            @if (auth()->user()->hasPermission('retail.sale.purchase-order.index'))
                <li>
                    <a class="dropdown-item d-inline-flex gap-2 align-items-center"
                        href="{{ route('retail.sale.purchase-order.show', ['order' => $item->id]) }}">
                        <i class="fa-solid fa-eye"></i>
                        Xem chi tiết
                    </a>
                </li>
            @endif
            @if ($saleAdminStep->status != 'confirmed' || !$hgfAdminStep->status != 'confirmed')
                @if (auth()->user()->hasPermission('retail.sale.purchase-order.edit'))
                    <li>
                        <a class="dropdown-item d-inline-flex gap-2 align-items-center"
                            href="{{ route('retail.sale.purchase-order.edit', ['order' => $item->id]) }}">
                ...
                @if (auth()->user()->isSuperUser())
                    <li>
                        <button class="dropdown-item d-inline-flex gap-2 align-items-center cancel-btn"
                            data-id="{{ $item->id }}">
                            <span class="icon-tabler-wrapper">
            ...
            @if (auth()->user()->hasPermission('retail.sale.purchase-order.send'))
                ...
                @endif
            @endif
            @if (auth()->user()->hasPermission('retail.sale.purchase-order.confirm'))
                @if ($saleAdminStep->status == 'pending')
                    <li>
                        <button class="dropdown-item d-inline-flex gap-2 align-items-center update_step_btn"
                            data-action="{{ Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_MANAGER_CONFIRM_ORDER }}"
                            data-status="confirmed" data-type="next" data-order-id="{{ $item->id }}">
                            <span class="icon-tabler-wrapper">
            ...
            @if (auth()->user()->hasPermission('retail.sale.quotation.index'))
                @if ($item->quotation)
                    <li>
                        <a href="{{ route('retail.sale.quotation.show', ['quotation' => $item->quotation->id]) }}"
                ...
                @endif
            @endif
            @if (auth()->user()->hasPermission('retail.sale.production.index'))
                @if ($item->production)
                    <li>
                        <a href="{{ route('retail.sale.production.show', ['production' => $item->production->id]) }}"
                            class="dropdown-item d-inline-flex gap-2 align-items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="19" viewBox="0 0 16 19"
                                fill="none">
                                <path
