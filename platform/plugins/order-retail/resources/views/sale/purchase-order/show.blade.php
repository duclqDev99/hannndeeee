...
@section('content')
    @php
        ...
        $saleAdminStep = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_MANAGER_CONFIRM_ORDER, $order->id);
            ...
            <div class="card-body overflow-auto">
                <div class="mb-3">
                    ...
                    <div class="mb-1">
                        ...
                        @if ($saleStep->status == 'sended')
                            <span>
                                {{ $saleStep?->handler?->name }}.
                                {{ \Carbon\Carbon::parse($saleStep->handled_at)->format('d-m-Y, H:i') }}
                            </span>
                        @else
                            <span class="badge bg-secondary text-white">Chưa gửi yêu cầu</span>
                        @endif
                    </div>
                    <div class="mb-1">
                        <span class="fw-bold">Sale Admin:</span>
                        @if ($saleAdminStep->status == 'confirmed')
                            <span class="badge bg-success text-white">Đã xác nhận</span>
                            <span>
                                bởi {{ $saleAdminStep?->handler?->name }}.
                                {{ \Carbon\Carbon::parse($saleAdminStep->handled_at)->format('d-m-Y, H:i') }}
                            </span>
                        @else
                            <span class="badge bg-secondary text-white">Chưa xác nhận</span>
                        @endif
                    </div>

                    <div class="mb-1">
                        <span class="fw-bold">HGF Admin:</span>
                    ...
                    <tbody id="product-preview">
                        ...
                        <tr>
                            ...
                            <td colspan="1" style="border-left: none !important">
                                <span class="fw-bold uppercase">{{ number_format($order->amount) }}đ</span>
                            </td>
                        </tr>
