@php
    $saleSend = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_SEND_PRODUCTION, $item->order->id);
@endphp
...
<div class="table-actions">
    <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdown-actions-57e516785256609e1606c1598309234b-120" style="">
            ...
            {{-- <li>
                <a class="dropdown-item d-inline-flex gap-2 align-items-center" href="{{ route('retail.sale.production.edit', ['production'=> $item->id]) }}">
            ...
            @if (auth()->user()->hasPermission('retail.sale.production.send'))
                @if ($saleSend->status == 'pending' || $saleSend->status == 'refused')
                    <li>
                        <button class="dropdown-item d-inline-flex gap-2 align-items-center update_step_btn"
                            data-action="{{ Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_SEND_PRODUCTION }}"
            ...
            @if (auth()->user()->hasPermission('retail.sale.production.index'))
                <li>
                    <a href="{{ route('retail.sale.production.show', ['production' => $item->id]) }}"
