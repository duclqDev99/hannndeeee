...
@section('content')
    @php
        $saleStep = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SEND_QUOTATION, $quotation->order->id);
        $saleManagerStep = get_action(
            \Botble\OrderStepSetting\Enums\ActionEnum::RETAIL_SALE_MANAGER_CONFIRM_QUOTATION,
            $quotation->order->id,
        );

        $paramCode = str_replace('#', '', $quotation->order->code);
        $route = route('retail.sale.quotation.index') . '?order_code=' . $paramCode;
        ...
        <div class="card position-relative">
            ...
            <div class="card-body overflow-auto">
                <div class="mb-3">
                    ...
                    <div class="mb-1">
                        @if ($saleManagerStep->status == 'confirmed')
                            <span class="badge bg-success text-white">Đã xác nhận</span>
                            <span>
                                bởi {{ $saleManagerStep?->handler?->name }}.
                                {{ \Carbon\Carbon::parse($saleManagerStep?->handled_at)->format('d-m-Y, H:i') }}
                            </span>
                        @else
                            <span class="badge bg-secondary text-white">Chưa thanh toán</span>
                        @endif
