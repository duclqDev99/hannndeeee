    ...
    methods: {
        ...
        async insertProductAgent() {
            try {
                ...
                const response = await axios.post(route('agent-warehouse.store-product-manual'),{
                    agent_warehouse: this.agentWarehouse['id'],
                    products: filteredChildProducts,
                });
                window.location.href = route('agent-warehouse.detail-odd', this.agentWarehouse['id']);
