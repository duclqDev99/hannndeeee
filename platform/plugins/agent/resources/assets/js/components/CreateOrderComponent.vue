                                ...
                                <div v-else class="list-group list-group-flush overflow-auto" style="max-height: 25rem">
                                    <div class="list-group-item cursor-pointer" v-ec-modal.add-customer>
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col">
                                                <span>{{ __('Tạo mới khách hàng') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <a
...
export default {
    ...
    methods: {
        loadListAgent: function(){
            let context = this;
            axios.get(route('agent.get-list-agent-for-user'))
            .then((res) => {
                console.log(res);
                context.listAgent = res.data.data
                if (context.listAgent.length > 0) {
                    this.agent_id = context.listAgent[0].id;
                }
            })
            .catch((error) => {
        ...
        checkDataBeforeCreateOrder: function (data = {}, onSuccess = null, onError = null) {
            ...
            axios
                .post(route('agent.orders.check-data-before-create-order'), formData, {
        ...
        createOrder: function (event, paid = false) {
            ...
            axios
                .post(route('agent.orders.create'), formData)
                .then((res) => {
                    let data = res.data.data
                    if (res.data.error) {
                        Botble.showError(res.data.message)
                        ...
                        setTimeout(() => {
                            window.location.href = route('agent.orders.edit', data.id)
                        }, 1000)
                    }
                })
                .catch((res) => {
        ...
        createNewCustomer: function (event) {
            ...
            axios
                .post(route('agent.orders.customers.create-customer-when-creating-order'), {
