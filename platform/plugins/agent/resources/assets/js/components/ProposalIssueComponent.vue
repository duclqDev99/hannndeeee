        ...
        <div class="flexbox">
            ...
            <div class="flex-content">
                ...
                <div>
                    ...
                    <mutil-select :products="dataList" @select="onSelect" :placeholder="placeholderText" :items="items"
                       >
                    </mutil-select>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async getProductInFormProposal(idProposal) {
            try {
                const res = await axios.get(route('agent-proposal-issue.proposal', { id: idProposal }));
                const data = res.data;
                this.agentIssue = data.agent;
        ...
        checkQuantity(item, event) {
            ...
            if (newValue < min) {
                ...
                this.alertWarning('Số lượng vượt quá lô trong kho')
            } else {
        ...
        changeWarehouseIssue() {
            ...
            axios.get(route('agent.products.get-product-in-agent', { id: this.warehouseIssue }))
                .then(res => {
                    this.dataList = res.data.data.map(item => {
                        ...
                        return {
                            ...
                            'sku': item.sku,
                            'quantityStock': item.product_agent[0].pivot.quantity_qrcode,
                            'is_batch':  0,

                            'id': item.id,
                            // ...item,
                            // 'id': item.id,
                            // 'image': item.product[0].images[0] ? item.product[0].images[0] : '',
                            // 'sku': item.product[0].sku,
                            // 'quantityStock': item.quantity_qrcode,
                            // 'name': name,
                            // 'is_batch': 0,
                            // 'parent_product': item.product[0].parent_product[0],
                            // 'quantity': 1
        ...
        changeAgentWarehouse() {
            ...
            this.dataList = []
            this.items = []
            this.agentIssue = $('#agent_id').val();
            this.fetchWarehouseByAgent();
        },
        reset() {
            this.items = []
        },
        fetchWarehouseByAgent(idWarehouse = null, dataProduct = null) {
            const dropdown = $('#warehouse_issue_id');
            dropdown.empty();
            const data = this.allAgentWarehouse.map(item => {
                if (this.agentIssue == item.agent_id) {
                    return {
                        'id': item.id,
                        'name': item.name,
                    };
            ...
            // axios.get(route('agent-warehouse.get-warehouse-by-agent', { id: idAgent }))
            //     .then(res => {
            //         const data = res.data
            //         this.updateDropdown(data, idWarehouse);
            //     })
            //     .catch(error => {
        ...
        submitForm(e) {
            e.preventDefault();
            const form = document.getElementById('botble-agent-forms-angent-proposal-issue-form');
            let isValid = true;
            ...
            if ($('#title').val() == '') {
                isValid = false;
                this.alertWarning('Phải nhập tiêu đề đơn nhập kho');
                return;
            }
