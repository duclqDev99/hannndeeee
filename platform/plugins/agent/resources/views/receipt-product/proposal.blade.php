...
<style>
                                ...
                                <form action="{{ route('proposal-receipt-products.approved', $proposal) }}" method="post">
                                    ...
                                    <div class="wrapper-content">
                                        <div class="pd-all-20">
                                            <div class="card-header justify-content-between">
                                                <div class="flexbox-auto-right mr5">
                                                    <label $class="title-product-main text-no-bold">{{ __('Phiếu đề xuất nhập kho') }}
                                                        <strong>{{ get_proposal_receipt_product_code($proposal->proposal_code) }}</strong> 
                                                        Người đề xuất: <strong>{{$proposal->invoice_issuer_name}},</strong>
                                                        lúc <span>{{ date('d/m/Y', strtotime($proposal->created_at)) }}</span>
                                                    </label>
                                                    <div>Kho: <strong>{{ $proposal->warehouse_name }}</strong></div>
                                                    <div>
                                                        Loại nhập kho:
                                                        @if ($proposal->is_warehouse == 'warehouse')
                                                            Nhập lô từ kho {{ $proposal->wh_departure_name }}
                                                        @elseif ($proposal->is_warehouse == 'warehouse-odd')
                                                            Nhập lẻ từ kho {{ $proposal->wh_departure_name }}
                                                        @else
                                                            Nhập kho hàng tồn
                                                        @endif
                                                    </div>
                                                            ...
                                                            <tr class="item__product">
                                                                ...
                                                                <td class="p-3">
                                                                    <div class="inline_block">
                                                                        <input type="number" class="widget__price" name="product[{{ $product->id }}][product_price]" value="{{$product->price}}" hidden>
                                                                ...
                                                                <td class="p-3">
                                                                    <div class="d-flex align-items-center">
                                                                        @if($proposal->is_warehouse == 'warehouse')
                                                                            <span style="white-space: nowrap;">SL trong kho: {{ !empty($proposal->wh_departure_id) ? ProductBatch::where([
                                                                            ...
                                                                            ])->groupBy('product_parent_id')->count() : '0' }} lô</span>
                                                                        @else
                                                                            <span style="white-space: nowrap;">SL trong kho: {{ !empty($proposal->wh_departure_id) ? QuantityProductInStock::where(['product_id' => $product->product_id, 'stock_id' => $proposal->wh_departure_id])->first()->quantity : '0' }}</span>
                                                                        @endif
                                                                    </div>
                                                                </td>
                                                                <td class="p-3" width="200px">
                                                                    <div class="d-flex align-items-center">
                                                                        ...
                                                                        <input type="number" class="value__total__price" value="{{$product->price * $product->quantity}}" hidden>

                                                                        @if($proposal->is_warehouse == 'warehouse')
                                                                            <input data-quantity="{{ !empty($proposal->wh_departure_id) ? ProductBatch::where([
                                                                                'warehouse_type' => WarehouseFinishedProducts::class,
                                                                                'warehouse_id' => $proposal->wh_departure_id,
                                                                                'product_parent_id' => $product->product_id
                                                                            ])->groupBy('product_parent_id')->count() : '0' }}" type="number" name="product[{{$product->id }}][quantity]" class="form-control widget__quantity" value="{{ $product->quantity }}" min="1" placeholder="0" required>
                                                                        @else
                                                                            <input data-quantity="{{ !empty($proposal->wh_departure_id) ? QuantityProductInStock::where(['product_id' => $product->product_id, 'stock_id' => $proposal->wh_departure_id])->first()->quantity : '0' }}" type="number" name="product[{{$product->id }}][quantity]" class="form-control widget__quantity" value="{{ $product->quantity }}" min="1" placeholder="0" required>
                                                                        @endif
                                        ...
                                        <div class="card-body pd-all-20 p-none-t">
                                            ...
                                            <div class="mt10">
                                                <a href="#" class="btn btn-danger" data-toggle="modal" data-target=".bg-modal-cancel">Huỷ đơn</a>
    ...
    <div class="modal fade bg-modal-cancel" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
