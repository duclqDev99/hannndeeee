@php
    ...
    if ($item->status == ProposalProductEnum::PENDING) {
        ...
        // if (Auth::user()->hasPermission('proposal-agent-receipt.approve') || Auth::user()->hasPermission('agent.all')) {
        //     $actionApprove =
        //         '
        //                 <li> <a data-bs-toggle="tooltip" class="dropdown-item text-success" href="' .
        //         route('proposal-agent-receipt.approve', $item) .
    ...
    if ($actionApprove == '') {
        $actionView =
            '
                    ...
                    href="' .
            route('proposal-agent-receipt.view', $item->id) .
