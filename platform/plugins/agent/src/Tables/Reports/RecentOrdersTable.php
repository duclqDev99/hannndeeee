...
class RecentOrdersTable extends TableAbstract
{
    ...
    {
        $this->setOption('id','botble-agent-tables-reports-recent-orders-table');
    ...
    public function columns(): array
    {
        return [
            LinkableColumn::make('code')
                ...
                ->width(20),
            Column::make('user_id')
                ->title(trans('plugins/agent::order.customer_label'))
                ->alignStart(),
            Column::formatted('amount')
                ->title(trans('plugins/agent::order.amount')),
            Column::make('payment_method')
                ->name('payment_id')
                ->title(trans('plugins/agent::order.payment_method'))
                ->alignStart(),
            Column::make('payment_status')
                ->name('payment_id')
                ->title(trans('plugins/agent::order.payment_status_label')),
            StatusColumn::make(),
