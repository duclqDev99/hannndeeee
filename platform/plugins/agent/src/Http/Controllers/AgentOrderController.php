...
use Botble\Agent\Http\Requests\AgentAddCustomerWhenCreateOrderRequest;
use Botble\Agent\Http\Requests\AgentCreateOrderRequest;
...
use Botble\Agent\Tables\AgentOrderTable;
...
class AgentOrderController extends BaseController
{
    ...
    protected function breadcrumb(): Breadcrumb
    {
        return parent::breadcrumb()
            ->add('Quản lí đơn hàng', route('agent.orders.index'));
    }

    public function index(AgentOrderTable $dataTable, BaseHttpResponse $response)
    {
        $this->pageTitle(trans('plugins/agent::order.menu'));

        Assets::addScriptsDirectly([
            'vendor/core/plugins/agent/js/scan-mobile.js',
        ]);
        ...
        if (count($agentUser) <= 0) {
            return $response
                ...
                ->setError()
                ->setMessage(trans('Bạn không thuôc đại lý nào!!!'));
        }
        $agentList = Agent::query()
            ->wherePublished()
        ...
        // return view('plugins/agent::orders.intro', compact('agent_id', 'agentList', 'nameAgent', 'dataTable'));

        return $dataTable->renderTable();
    }

    public function checkProductOrderInAgent(Request $request, BaseHttpResponse $response)
    {
        $warehouse_id = $request->warehouse_id;
        $warehouse_type = $request->warehouse_type;
        $agentId = $request->agent_id;
        if ($warehouse_type != AgentWarehouse::class) {
            return $response
                ->setError()
                ->setMessage(trans('Sản phẩm không thuộc đại lý này!!!'));
        }
        $warehouse = AgentWarehouse::query()
            ->wherePublished()
        ...
        if ($warehouse == 0) {
            return $response
                ->setError()
                ->setMessage(trans('Không có sản phẩm này!!!'));
        } else {
            return $response
                ->setMessage(trans('Thêm sản phẩm thành công!!!'));
        }
    }

    public function submitPayment(Request $request, BaseHttpResponse $response)
    {
        $total = $request->input('total');
        $productQr = $request->input('qr_ids');
        $agentId = $request->input('agent_id');
        $customer_id = $request->input('customer_id');
        DB::beginTransaction();
        try {

            $agentStatistics = AgentStatistics::firstOrCreate(
                ['where_type' => Agent::class, 'where_id' => $agentId],
                ['created_at' => Carbon::now()]
            );

            $agentStatistics->increment('revenue', $total);
            $agentStatistics->increment('quantity_product', count($productQr));
            ...
            if ($order) {
                ...
                $agentOrder->amount = $total;
                $agentOrder->list_id_product_qrcode = $productQr;
                $agentOrder->status = OrderStatusEnum::COMPLETED();
                $agentOrder->where_type = Agent::class;
                $agentOrder->where_id = $agentId;
                $agentOrder->customer_id = $customer_id;
                $agentOrder->user_id = Auth::id();
                $agentOrder->order_id = $order->getKey();
                $agentOrder->created_at = Carbon::today();
                $agentOrder->code = $order->code;
                $agentOrder->save();
            }

                // AgentStatistics::insert([
                //     'revenue' => $total,
                //     'quantity_product' => count($productQr),
                //     'where_type' => Agent::class,
                //     'where_id' => $agentId,
                //     'created_at' => Carbon::today(),
                // ]);
            $totalProducts = ProductQrcode::whereIn('id', $productQr)
                                ->select('reference_id', DB::raw('count(*) as total'))
                                ->groupBy('reference_id')
                                ->get();
            ProductQrcode::whereIn('id', $productQr)->update(['status' => QRStatusEnum::SOLD]);
            forEach($totalProducts as $totalProduct){
                AgentProduct::where('product_id', $totalProduct->reference_id)
                    ->decrement('quantity_qrcode', $totalProduct->total);
            }

            $this->checkProductBatch($productQr);
            // ProductQrcode::query()->whereIn('id', $productQr)->update(['status' => QRStatusEnum::SOLD]);
            DB::commit();
            return $response
                ->setMessage(trans('Thanh toán thành công!!!'));
        } catch (Exception $e) {
            DB::rollBack();
            return $response
                ->setError()
    ...
    public function detail($id, BaseHttpResponse $response)
    {
        ...
        if(!$order){
            return $response
                ->setError()
                ->setMessage(trans('Đơn không tồn tại!!!'));
        }

        $agentName = Agent::where('id', $order?->where_id)->select('name')->first();
        $uniqueProducts = ProductQrcode::query()
            ->whereIn('id', $order->list_id_product_qrcode)
            ->with('reference.variationProductAttributes')
            ->get();
        $totalOrder = count($uniqueProducts);
        $products = $uniqueProducts->groupBy('reference_id')->map(function ($group) {
            $product = $group->first();
            $product->total = $group->count();
            return $product;
        })->values();

        $this->pageTitle(trans('chi tiết đơn: ' . $order->code));
        return view('plugins/agent::orders.detail', compact('order', 'products', 'agentName', 'totalOrder'));
    }

    public function create(BaseHttpResponse $response, Request $request)
    {
        $this->pageTitle(trans('plugins/agent::order.menu'));
        ...
        $nameAgent = count($agentList) > 1 ? null : reset($agentList);
        $agent_id = $request->select_id;

        return view('plugins/agent::orders.intro', compact('agent_id', 'agentList', 'nameAgent'));
    }

    public function store(AgentCreateOrderRequest $request)
    {
        ...
        if ($order) {
            OrderHistory::query()->create([
                ...
                'description' => trans('plugins/agent::order.create_order_from_admin_page'),
            ...
            OrderHistory::query()->create([
                ...
                'description' => trans('plugins/agent::order.order_was_verified_by'),
                'order_id' => $order->getKey(),
                'user_id' => $userId,
            ]);
            ...
            if (is_plugin_active('payment')) {
                ...
                if ($paymentStatus == PaymentStatusEnum::COMPLETED) {
                    ...
                    OrderHistory::query()->create([
                        ...
                        'description' => trans('plugins/agent::order.payment_was_confirmed_by', [
    ...
    {
        abort_if(
            is_null($order->agentOrder?->where?->users) ||
                (!is_null($order->agentOrder?->where?->users) && !$order->agentOrder?->where?->users->contains('id', request()->user()->id)),
            403,
            'Không có quyền'
        );
        ...
        $this->pageTitle(trans('plugins/agent::order.edit_order', ['code' => $order->code]));

        $weight = number_format(EcommerceHelper::validateOrderWeight($order->products_weight));

        $defaultStore = get_primary_store_locator();

        return view('plugins/agent::orders.edit', compact('order', 'weight', 'defaultStore'));
    }
    ...
    public function postConfirm(Request $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.confirm_order_success'));
    }
    ...
    public function postResendOrderConfirmationEmail(Order $order)
    {
        ...
        if (!$result) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage(trans('plugins/agent::order.error_when_sending_email'));
        }

        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.sent_confirmation_email_success'));
    }
    ...
    public function getShipmentForm(
        ...
        if ($request->has('view')) {
            return view(
                'plugins/agent::orders.shipment-form',
    ...
    public function postCreateShipment(Order $order, CreateShipmentRequest $request)
    {
        ...
        $result = $result->setMessage(trans('plugins/agent::order.order_was_sent_to_shipping_team'));
        ...
        if (!$result->isError()) {
            ...
            OrderHistory::query()->create([
                ...
                'description' => $result->getMessage() . ' ' . trans('plugins/agent::order.by_username'),
                'order_id' => $order->getKey(),
                'user_id' => Auth::id(),
            ]);
            ...
            ShipmentHistory::query()->create([
                ...
                'description' => trans('plugins/agent::order.shipping_was_created_from'),
    ...
    public function postCancelShipment(Shipment $shipment)
    {
        ...
        OrderHistory::query()->create([
            ...
            'description' => trans('plugins/agent::order.shipping_was_canceled_by'),
        ...
        return $this
            ...
            ->setMessage(trans('plugins/agent::order.shipping_was_canceled_success'));
    }
    ...
    public function postUpdateShippingAddress(OrderAddress $address, AddressRequest $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setData([
                'line' => view('plugins/agent::orders.shipping-address.line', compact('address'))->render(),
                'detail' => view('plugins/agent::orders.shipping-address.detail', compact('address'))->render(),
            ])
            ->setMessage(trans('plugins/agent::order.update_shipping_address_success'));
    }
    ...
    public function postUpdateTaxInformation(OrderTaxInformation $taxInformation, Request $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setData(view('plugins/agent::orders.tax-information.detail', ['tax' => $taxInformation])->render())
            ->setMessage(trans('plugins/agent::order.tax_info.update_success'));
    }
    ...
    public function postCancelOrder(Order $order)
    {
        ...
        OrderHistory::query()->create([
            'action' => 'cancel_order',
            'description' => trans('plugins/agent::order.order_was_canceled_by'),
            'order_id' => $order->id,
            'user_id' => Auth::id(),
        ]);
        ...
        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.cancel_success'));
    }
    ...
    public function postConfirmPayment(Order $order)
    {
        ...
        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.confirm_payment_success'));
    }
    ...
    public function postRefund(Order $order, RefundRequest $request)
    {
        if (is_plugin_active('payment') && $request->input(
            ...
            return $this
                ...
                ->setError()
                ->setMessage(
                    trans('plugins/agent::order.refund_amount_invalid', [
                        'price' => format_price(
                            $order->payment->amount - $order->payment->refunded_amount,
        ...
        foreach ($request->input('products', []) as $productId => $quantity) {
            ...
            if ($quantity > ($orderProduct->qty - $orderProduct->restock_quantity)) {
                $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(trans('plugins/agent::order.number_of_products_invalid'));
        ...
        if (!$payment) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage(trans('plugins/agent::order.cannot_found_payment_for_this_order'));
        }
        ...
        if ($refundAmount > 0) {
            OrderHistory::query()->create([
                'action' => 'refund',
                'description' => trans('plugins/agent::order.refund_success_with_price', [
        ...
        $response->setMessage(trans('plugins/agent::order.refund_success'));
    ...
    public function postApplyCoupon(ApplyCouponRequest $request, HandleApplyCouponService $handleApplyCouponService)
    {
        ...
        return $this
            ...
            ->setMessage(
                trans(
                    'plugins/agent::order.applied_coupon_success',
    ...
    public function getReorder(Request $request)
    {
        if (!$request->input('order_id')) {
            return $this
                ...
                ->setMessage(trans('plugins/agent::order.order_is_not_existed'));
        }

        $this->pageTitle(trans('plugins/agent::order.reorder'));
    ...
    {
        $this->pageTitle(trans('plugins/agent::order.incomplete_order'));
    ...
    public function getViewIncompleteOrder(Order $order)
    {
        ...
        return view('plugins/agent::orders.view-incomplete-order', compact('order', 'weight'));
    }
    ...
    public function markIncompleteOrderAsCompleted(Order $order)
    {
        DB::transaction(function () use ($order) {
            ...
            $order->histories()->create([
                ...
                'description' => trans('plugins/agent::order.mark_as_completed.history', [
        ...
        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.mark_as_completed.success'))
            ->setData([
    ...
    public function postSendOrderRecoverEmail(Order $order)
    {
        ...
        try {
            ...
            return $this
                ->httpResponse()->setMessage(trans('plugins/agent::order.sent_email_incomplete_order_success'));
        } catch (Exception $exception) {
            return $this
                ->httpResponse()
                ->setError()
    ...
    protected function getDataBeforeCreateOrder(Request $request): array
    {
                ...
                if (!$inputOptions) {
                    ...
                    foreach ($requiredOptions as $requiredOption) {
                        if (!Arr::get($inputOptions, $requiredOption->id . '.values')) {
                            $isError = true;
                            $message[] = trans(
                                'plugins/agent::product-option.add_to_cart_value_required',
        ...
        if ($shippingType == 'free-shipping') {
            $shippingMethodName = trans('plugins/agent::order.free_shipping');
    ...
    public function generateInvoice(Order $order)
    {
        ...
        return $this
            ->httpResponse()
            ->setMessage(trans('plugins/agent::order.generated_invoice_successfully'));
    }
    public function postCreateCustomerWhenCreatingOrder(AgentAddCustomerWhenCreateOrderRequest $request)
    {
