...
class AgentCustomerController extends BaseController
{
    use CustomersAppTrait;
    public function store(AgentAddCustomerWhenCreateOrderRequest $request)
    {
        ...
        try{
            ...
            AgentCustomer::updateOrCreate(
                ['customer_id' => $customer->id, 'where_type' => Agent::class, 'where_id' => $agent_id], // Điều kiện tìm kiếm
                ['customer_id' => $customer->id, 'where_type' => Agent::class, 'where_id' => $agent_id] // Dữ liệu cần cập nhật hoặc tạo mới
            );
