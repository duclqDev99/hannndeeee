        ...
        <div class="row row-cards">
                            ...
                            <div class="col-md-6 offset-md-6">
                                    ...
                                    <x-core::table.body>
                                        ...
                                        @if ($order->proof_file && Storage::disk('local')->exists($order->proof_file))
                                            <x-core::table.body.row>
                                                ...
                                                <x-core::table.body.cell>
                                                    <a href="{{ route('orders.download-proof', $order->id) }}"
                                                        target="_blank">
                                                        {{ $order->proof_file }}
                                                    </a>
                                                </x-core::table.body.cell>
                                ...
                                <div class="btn-list justify-content-end my-3">
                                    @if ($order->isInvoiceAvailable())
                                        ...
                                        <x-core::button tag="a" :href="route('orders.generate-invoice', $order->id)" target="_blank"
                                            icon="ti ti-download">
                                            {{ trans('plugins/ecommerce::order.download_invoice') }}
                                        </x-core::button>
                                    @else
                                        <x-core::button tag="a" :href="route('orders.invoice.generate', $order->id)" target="_blank"
                                    ...
                                    {{-- <div class="btn-list">
                                        @if (!$order->payment->status || in_array($order->payment->status, [Botble\Payment\Enums\PaymentStatusEnum::PENDING]))
                                            <x-core::button type="button" color="info"
                                                class="btn-trigger-confirm-payment" :data-target="route('orders.confirm-payment', $order->id)">
            ...
            <div class="col-md-3">
                <x-core::card>
                    ...
                    <x-core::card.footer>
                        <div class="btn-list">
                            <x-core::button tag="a" :href="route('orders.reorder', ['order_id' => $order->id])">
                                {{ trans('plugins/ecommerce::order.reorder') }}
                            </x-core::button>
                            @if ($order->canBeCanceledByAdmin())
                                <x-core::button type="button" :data-target="route('orders.cancel', $order->id)" class="btn-trigger-cancel-order">
...
@if (is_plugin_active('payment'))
    <x-core::modal.action id="confirm-payment-modal" type="info" :title="trans('plugins/ecommerce::order.confirm_payment')" :description="trans('plugins/ecommerce::order.confirm_payment_confirmation_description', [
        'method' => $order->payment->payment_channel->label(),
    ])"
        :submit-button-attrs="['id' => 'confirm-payment-order-button']" :submit-button-label="trans('plugins/ecommerce::order.confirm_payment')" />
