...
@section('content')
    ...
    @if (Cart::instance('cart')->isNotEmpty())
        ...
        <x-core::form :url="route('public.checkout.process', $token)" id="checkout-form" class="checkout-form payment-checkout-form">
            ...
            <div class="container position-relative" id="main-checkout-product-info">
                <div class="row">
                    <div class="order-1 order-md-2 col-lg-5 col-md-6 right position-sticky top-0">
                        ...
                        <div class="mt-3 mb-5">
                            @include(
                                'plugins/ecommerce::themes.discounts.partials.form',
                                compact('discounts'))
                        </div>
                    </div>
                    ...
                    <div class="col-lg-7 col-md-6 left">
                        ...
                        <div class="form-checkout" style="position: relative!important;background: #fff!important; z-index: 99!important;padding-top: 20px;">
                            ...
                            @if ($isShowAddressForm)
                                <div class="mb-4">
                                    ...
                                    <input id="save-shipping-information-url" type="hidden"
                            ...
                            @endif
                            @if (!in_array('showroom', EcommerceHelper::getHiddenFieldsAtCheckout()))
                            <div @class([
                                'col-12' => !in_array(
                                    'showroom',
                                ...
                                <div class="form-group mb-3 @error('address.showroom') has-error @enderror">
                                    <div class="select--arrow form-input-wrapper">
                                        <select class="form-control" id="address_showroom" name="address[showroom]" data-type="showroom"
                                            data-using-select2="false" data-url="{{ route('get-districts-by-state') }}" required>
                                            <option value="">{{ __('Chọn showroom...') }}</option>
                                            @foreach ($showrooms as $showroom)
                                                <option data-lat="{{ $showroom->lat }}" data-lon="{{$showroom->lon}}" value="{{ $showroom->id }}"
                                                    @if (old('address.showroom', Arr::get($sessionCheckoutData, 'showroom')) == $showroom->id) selected @endif>{{ $showroom->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <x-core::icon name="ti ti-chevron-down" />
                                        <label for="address_showroom">{{ __('Showroom mua hàng') }}</label>
                                    </div>
                                    {!! Form::error('address.showroom', $errors) !!}
                                </div>
                            </div>
                        @endif
                                                ...
                                                <div class="card p-3">
                                                    <div class="d-flex gap-3 pb-1">
                                                        @foreach ($shipping as $shippingItem)
                                                            <button style="width: 65px; height: 60px;"
                                                                class="form-control overflow-hidden btn form-control d-flex align-items-center justify-content-center"
                                                                type="button">
                                                                <label for="{{ $shippingItem['method_name'] }}"
                                                                    style=" cursor: pointer">
                                                                    <img style="width: 60px; height: 55px;"
                                                                        src="{{ $shippingItem['image'] }}"
                                                                        alt="{{ $shippingItem['title'] }}" />
                                                                    <input class="magic-radio shipping_method_radio"
                                                                        id="{{ $shippingItem['method_name'] }}"
                                                                        name="shipping_method" type="radio"
                                                                        value="{{ $shippingItem['method_name'] }}"
                                                                        data-title="{{ $shippingItem['title'] }}"
                                                                        data-get-rates-url="{{ $shippingItem['get_rates_url'] }}"
                                                                        @if (old('shipping_method', Arr::get($sessionCheckoutData, 'shipping_method')) == $shippingItem['method_name']) checked @endif>
                                                                </label>
                                                            </button>
                                                        @endforeach
                                                    </div>
                                                    @if ($shippingSelected)
                                                        <div class="card pt-3 border-0 shipping_options_wrapper">
                                                            <div>
                                                                <p class="mb-0 fw-bolder shipping_method_selected_title">
                                                                    {{ $shippingSelected['title'] }}
                                                                </p>
                                                            </div>
                                                            @if (isset($shippingSelected['options']) && count($shippingSelected['options']) > 0)
                                                                <ul
                                                                    class="mt-3 list-group shipping_options">
                                                                    @foreach ($shippingSelected['options'] as $option)
                                                                        <li class="list-group-item">
                                                                            <input class="magic-radio "
                                                                                id="shipping_option_{{ $option['value'] }}}"
                                                                                name="shipping_option" type="radio"
                                                                                value="{{ $option['value'] }}"
                                                                                data-price="{{ $option['fee'] }}"
                                                                                @if (!old('shipping_option', Arr::get($sessionCheckoutData, 'shipping_option'))) @if (Arr::get($option, 'is_default') == true) checked @endif
                                                                            @else
                                                                                @if (old('shipping_option', Arr::get($sessionCheckoutData, 'shipping_option')) == Arr::get($option, 'value')) checked @endif
                                                                                @endif
                                                                            >
                                                                            <label class="text-start"
                                                                                for="shipping_option_{{ $option['value'] }}}">
                                                                                {{ $option['name'] }} - <span
                                                                                    class="fw-bolder">{{ number_format($option['fee']) }}
                                                                                    VNĐ</span>
                                                                            </label>
                                                                        </li>
                                                                    @endforeach
                            ...
                            <div class="order-1 order-md-2 col-md-6">
                                @if (EcommerceHelper::isValidToProcessCheckout())
                                    <button class="btn payment-checkout-btn payment-checkout-btn-step float-end"
                                        ...
                                        data-error-header="{{ __('Error') }}" type="submit">
                                        {{ __('Checkout') }}
                                    </button>
                                @else
...
<style>
    .ring-4 {
        box-shadow: #38bdf8 0 0 0 calc(4px) #38bdf8;
    }
</style>
@stop
