...
class MainCheckout {
    ...
    init() {
            ...
            try {
                if ("geolocation" in navigator) {
                    ...
                    navigator.geolocation.getCurrentPosition(function (position) {
                        ...
                        // Ví dụ về tọa độ của bạn và một danh sách tọa độ
                        var yourLocation = { lat: position.coords.latitude, lon: position.coords.longitude };

                        const showroomOptions = $('select[data-type="showroom"] option');
                        var locations = [];

                        $(showroomOptions).each(function(){
                            locations.push({lat: $(this).data('lat'), lon: $(this).data('lon'), value: $(this).prop('value')});
                        })
                        ...
                        locations.forEach(function (loc) {
                            var distance = getDistance(yourLocation.lat, yourLocation.lon, loc.lat, loc.lon);
                        ...
                        if(closest){
                            $('select[data-type="showroom"]').val(closest.value);
                            $('select[data-type="showroom"]').trigger('change');
                        }
                ...
                console.log('Lấy tọa độ thất bại. Người dùng không cấp quyền truy cập!')
            }
        }

        let loadShippingFeeAtTheFirstTime = () => {
            let shippingMethod = $(document).find('input[name=shipping_method]:checked').first()
            let shippingOption = $(document).find('input[name=shipping_option]:checked').first()
            ...
            if (shippingMethod.length) {
                ...
                let params = {
                    ...
                    shipping_option: shippingOption.val(),
                    payment_method: '',
                }
        ...
        $(document).on('change', 'input[name=shipping_option]', (event) => {
            // Fixed: set shipping_option value based on shipping_method change:
            const $this = $(event.currentTarget)
        ...
        let onChangeShippingForm = (event) => {
            $('.payment-checkout-btn-step').prop('disabled', true)
        ...
        $(document).on('change', '#address_showroom', (event) => {
            onChangeShippingForm(event)
        })
    }
}
