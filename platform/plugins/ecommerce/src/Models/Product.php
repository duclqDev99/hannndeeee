...
class Product extends BaseModel
{
    ...
    {
        return $this->morphMany(ProductQrcode::class, 'reference', 'reference_type', 'reference_id');
    }
    public function getQrcodeIdentifierAttribute($identifier)
    {
        $qrcode = $this->qrcodes->filter(function ($qrcode) use ($identifier) {
            return false !== stripos($qrcode->identifier, $identifier);
        })->first();
        return $qrcode ? $qrcode : null;
    }

    public function nameHomepage(): Attribute
    {
        return Attribute::get(function () {
            return $this->home_display_name ?? $this->name;
        });
    }
}
