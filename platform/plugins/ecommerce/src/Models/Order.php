...
class Order extends BaseModel
{
    ...
    public function getShowroomIdAttribute()
    {
        return ShowroomOrder::select(['where_id'])->where('order_id', $this->id)->first()->where_id;
    }

    public function getIsFromHomeAttribute(){
        return $this->payment->customer_type == Customer::class;
    }

    public function getIsScannedAttribute()
    {
        return is_array($this->showroomOrder->list_id_product_qrcode)
            && count($this->showroomOrder->list_id_product_qrcode) > 0;
    }

    public function showroomOrder(): HasOne
    {
        return $this->hasOne(showroomOrder::class, 'order_id');
    }
