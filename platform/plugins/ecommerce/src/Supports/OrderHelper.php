...
class OrderHelper
{
    ...
    public function processAddressOrder(int|string $currentUserId, array $sessionData, Request $request): array
    {
        ...
        if (!empty($address)) {
            ...
            $addressData = [
                ...
                'order_id' => Arr::get($sessionData, 'created_order_id', 0),
                'province_id' => Arr::get($sessionData, 'province', null),
                'district_id' => Arr::get($sessionData, 'district', null),
                'ward_id' => Arr::get($sessionData, 'ward', null),
            ];
        }
    ...
    protected function createOrderAddress(array $data, ?array $sessionData = []): OrderAddress|bool
    {
        ...
        if (Arr::exists($data, 'address_id')) {
            $address = Address::find(Arr::get($data, 'address_id', null));
            if ($address) {
                Arr::set($data, 'province_id', $address->province_id);
                Arr::set($data, 'district_id', $address->district_id);
                Arr::set($data, 'ward_id', $address->ward_id);
            }
        } else {
            Arr::set($data, 'province_id', Arr::get($data, 'province', null));
            Arr::set($data, 'district_id', Arr::get($data, 'district', null));
            Arr::set($data, 'ward_id', Arr::get($data, 'ward', null));
        }
