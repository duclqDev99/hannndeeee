...
class EcommerceHelper
{
    ...
    public function getCustomerAddressValidationRules(string|null $prefix = ''): array
    {
        ...
        //     $prefix . 'district' => 'required|max:120',
        ...
        $rules = [
            ...
            $prefix . 'address' => 'required|max:120',
            $prefix . 'phone' => $this->getPhoneValidationRule(),
            $prefix . 'province' => 'required|exists:viettel_province,viettel_id',
            $prefix . 'district' => 'required|exists:viettel_district,viettel_id',
            $prefix . 'ward' => 'required|exists:viettel_wards,viettel_id',
            $prefix . 'showroom' => 'required|exists:showrooms,id', 
        ];
    ...
    public function getShippingData(
        ...
        if(Arr::get($session, 'address_id') != 'new'){
            $address_id = Arr::get($session, 'address_id');
            $address = Address::    find($address_id);

            if($address){
                Arr::set($session, 'province' , $address->province_id);
                Arr::set($session, 'district' , $address->district_id);
                Arr::set($session, 'ward' , $address->ward_id);
            }
        }

        
        $keys = ['name', 'company', 'address', 'country', 'state', 'city', 'zip_code', 'email', 'phone', 'province', 'district', 'ward', 'showroom'];
        ...
        $data = [
            ...
            'shipping_method_selected' => Arr::get($session, 'shipping_method', null),
            'shipping_option_selected' => Arr::get($session, 'shipping_option', null),
        ];
