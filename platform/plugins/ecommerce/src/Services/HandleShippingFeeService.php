...
class HandleShippingFeeService
{
    ...
    public function execute(array $data, string|null $method = null, string|null $option = null): array
    {
        ...
        // unset default shipping method
        Arr::forget($result, 'default');

        $result = apply_filters('handle_shipping_fee', $result, $data, $option);

        if ($method) {

            $options = Arr::get($result, "$method.options", []);
            
            if (! is_array($options) || ! count($options)) {
                return [];
            }

            $filtered = Arr::where($options, function ($rate) use($option) {
                return Arr::get($rate, 'value') == $option;
            });
