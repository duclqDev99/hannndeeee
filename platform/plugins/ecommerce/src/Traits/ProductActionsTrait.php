...
trait ProductActionsTrait
{
    ...
    protected function deleteVersionItem(int|string $variationId)
    {
        ...
        try{
            $variation = ProductVariation::query()->find($variationId);
            $product = Product::where('id', ($variation?->product_id))->with('variationProductAttributes')->first();
            ...
            $arrNoti = [
                'action' => 'Xóa biến thể cho',
                'permission' => "products.edit",
                'route' => route('products.edit', $variation?->configurable_product_id),
                'status' => 'Xóa'
            ];
            send_notify_cms_and_tele($product, $arrNoti);
            if (! $variation) {
                return true;
            }
    ...
    public function postAddVersion(ProductVersionRequest $request, int|string|null $id, BaseHttpResponse $response)
    {
        ...
        try{
            if (! empty($addedAttributes) && is_array($addedAttributes)) {
                ...
                $productVariation = ProductVariation::find($result['variation']?->id);

                $product = Product::where('id', ($productVariation?->product_id))->with('variationProductAttributes')->first();
                $arrNoti = [
                    'action' => 'Thêm mới biến thể cho',
                    'permission' => "products.edit",
                    'route' => route('products.edit', $result['variation']->configurable_product_id),
                    'status' => 'Thêm mới'
                ];
                send_notify_cms_and_tele($product, $arrNoti);
    ...
    public function postUpdateVersion(ProductVersionRequest $request, int|string $id, BaseHttpResponse $response)
    {
        ...
        if (! empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            $id = $productVariation?->product_id ?? $productVariation?->configurable_product_id;
            $product = Product::where('id', ($id))->with('variationProductAttributes')->first();
            $arrNoti = [
                'action' => 'Chỉnh sửa biến thể cho',
                'permission' => "products.edit",
                'route' => route('products.edit', $result['variation']->configurable_product_id),
                'status' => 'Chỉnh sửa'
            ];
