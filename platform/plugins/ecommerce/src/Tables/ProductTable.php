...
use Botble\Ecommerce\Actions\DetailAction;
...
class ProductTable extends TableAbstract
{
    public function setup(): void
    {
        $this
            ->model(Product::class)
            ->addActions([
                DetailAction::make()->route('products.detail')->permission('products.index'),
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('production_time', function (Product $item) {
                $value = $item?->production_time != null ? Carbon::parse($item->production_time)->format('m-Y') : '__' ;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('production_time')
                ->title(trans('plugins/ecommerce::products.form.production_time')),
    ...
    public function getFilters(): array
    {
        ...
        $data['is_show_home'] = [
            'title' => trans('Hiển thị trang chủ'),
            'type' => 'select',
            'choices' => [
                1 => 'Có',
                0 => 'Không'
