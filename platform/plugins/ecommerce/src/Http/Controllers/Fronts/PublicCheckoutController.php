...
class PublicCheckoutController extends BaseController
{
    ...
    public function getCheckout(
        ...
        if (Session::has('customer_phone_checkout')) {
            $sessionCheckoutData = OrderHelper::getOrderSessionData(Session::get('customer_phone_checkout'));
            Session::forget('customer_phone_checkout');
        }

        // $customer = Customer::where('phone', '')->first();
        ...
        foreach ($products as $product) {
            if ($product->isOutOfStock()) {
                return $this
                    ...
                    ->setNextUrl(route('public.cart'))
                    ->setMessage(
                        __('Product :product is out of stock!', ['product' => $product->original_product->nameHomepage])
                    );
            }
        }
        ...
        if (is_plugin_active('marketplace')) {
            ...
            $defaultShippingMethod = $request->input(
                'shipping_method',
                Arr::get($sessionCheckoutData, 'shipping_method', 'ghtk')
            );
            $defaultShippingOption = $request->input(
                'shipping_option',
                Arr::get($sessionCheckoutData, 'shipping_option', null)
            );
                ...
                if ($shipping) {
                    ...
                    if (!$defaultShippingMethod) {
                        $defaultShippingMethod = old(
                            'shipping_method',
                            Arr::get($sessionCheckoutData, 'shipping_method', $defaultShippingMethod)
                        );
                    }
                    ...
                    if ($defaultShippingMethod) {
                        $shippingSelected = Arr::get($shipping, $defaultShippingMethod);

                        if ($shippingSelected) {
                            foreach ($shippingSelected['options'] as $option) {
                                if (!$defaultShippingOption) {
                                    if ($option['is_default'])
                                        $shippingAmount = $option['fee'];
                                } else {
                                    if ($option['value'] == $defaultShippingOption)
                                        $shippingAmount = $option['fee'];
                                }
                            }
                        }
    ...
    protected function processOrderData(
            ...
            if ($request->input('create_account') == 1) {
                ...
                if (!auth('customer')->check()) {
                    $customer = Customer::query()
                        ->where('phone',  BaseHelper::clean($request->input('address.phone')))
                        ->orWhere('email',  BaseHelper::clean($request->input('address.email')))
                        ->first();
                    ...
                    $address = Address::query()
                        ->updateOrCreate(
                            [
                                ...
                                'province_id' => Arr::get($addressData, 'province'),
                                'district_id' => Arr::get($addressData, 'district'),
                                'ward_id' => Arr::get($addressData, 'ward'),
                            ],
                            array_merge($request->input('address'), [
                                ...
                                'district_id' => Arr::get($addressData, 'district'),
                                'ward_id' => Arr::get($addressData, 'ward'),
                                'showroom_id' =>  Arr::get($addressData, 'showroom'),
                            ])
                        );
        ...
        if (!isset($sessionData['created_order'])) {
            ...
            $request->merge([
                ...
                'shipping_method' => $request->input('shipping_method', ShippingMethodEnum::GHTK),
    ...
    public function postCheckout(
        ...
        $shippingMethodInput = $request->input('shipping_method', 'ghtk');
            ...
            if (!$shippingMethod) {
                throw ValidationException::withMessages([
                    'shipping_method' => trans(
                        'validation.exists',
                        ['attribute' => "Phương pháp vận chuyển không hợp lệ"]
                    ),
                ]);
            }

            $shippingAmount = Arr::get($shippingMethod, 'fee', 0);
        ...
        $showroomOrder = ShowroomOrder::query()->create([
            ...
            'where_id' => Arr::get($request, 'address.showroom', null),
            'list_id_product_qrcode' => json_encode([]),
            'list_id_product_qrcode_sale' => json_encode([]),
        ]);
        ...
        //         'status' => '1 đơn hàng mới từ trang chủ'
        //     ];
        ...
        OrderHistory::query()->create([
            ...
            'description' => __('Đơn hàng được tạo từ trang chủ'),
            'order_id' => $order->getKey(),
        ]);
        event(new CustomerCheckoutEvent($order, $sessionData));
        // if ($isAvailableShipping) {
        ...
        return $this
            ...
            ->setMessage(__('Checkout order successfully!'));
    }
