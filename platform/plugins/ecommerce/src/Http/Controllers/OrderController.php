...
class OrderController extends BaseController
{
    ...
    public function getShipmentForm(
        ...
        $shippingData = [
            'shipping_method_selected' => $order->shipping_method,
            'weight' => $weight,
            'order_total' => $order->amount,
            'address_to' => [
                'province' => $order->address->province_id,
                'district' => $order->address->district_id,
                'ward' => $order->address->ward_id,
        ...
        $pickShowroom = $order->showroomOrder->where;
        if ($request->has('view')) {
            return view(
                'plugins/ecommerce::orders.shipment-form',
                compact('order', 'weight', 'shipping', 'storeLocators', 'url', 'pickShowroom')
            );
        }
    ...
    public function postCreateShipment(Order $order, CreateShipmentRequest $request)
    {
        ...
        $customerAddress = [];
        Arr::set($customerAddress, 'name', $order->address->name);
        Arr::set($customerAddress, 'email', $order->address->email);
        Arr::set($customerAddress, 'tel', $order->address->phone);

        Arr::set($customerAddress, 'address', $order->address->address);
        Arr::set($customerAddress, 'province', $order->address->province_name);
        Arr::set($customerAddress, 'district', $order->address->district_name);
        Arr::set($customerAddress, 'ward', $order->address->ward_name);
        Arr::set($customerAddress, 'hamlet', "Khác");
        Arr::set($customerAddress, 'note', $request->note ?? '');
        Arr::set($customerAddress, 'pick_name', auth()->user()->name);

        $showroom = Showroom::find($order->showroom_id);
        $showroomPickAddress = null;

        if ($showroom) {
            $showroomPickAddress = $showroom->pickAddresses()->where('service_type', $request->method)->first();
            Arr::set($customerAddress, 'pick_address_id', $showroomPickAddress->pick_address_id);
        }



        if ($order->payment->payment_channel == PaymentMethodEnum::COD) {
            $pick_money = (int) $order->amount - (int) $order->shipping_amount;
            Arr::set($customerAddress, 'pick_money',  $pick_money);
        } else {
            if ($order->payment->status->getValue() !== PaymentStatusEnum::COMPLETED)
                return $result->setMessage("Đơn hàng chưa thanh toán!");
            Arr::set($customerAddress, 'pick_money', 0);
            Arr::set($customerAddress, 'is_freeship', true);
            Arr::set($shipment, 'payer',  ShippingPayerStatusEnum::SHOP);
        }
        ...
        $orderData = [
            ...
            "pick_address" => "Đường Lê Văn Khương",
            "pick_tel" => "0123456789",
            "value" => (int) $order->sub_total,
            "transport" => "road",
            "pick_option" => "cod",
            "deliver_option" => $request->input('option'),
            "tags" => [],
            "return_name" => auth()->user()->name,
            "return_address" => "Hẽm 119",
            "return_province" => "TP Hồ Chí Minh",
            "return_district" => "Quận 12",
            "return_tel" => str_replace('+', '', auth()->user()->phone),
            "return_email" => auth()->user()->email
        ] + $customerAddress;


        $products = $order->products->map(function ($item) {
            $attributes = Arr::get($item->options, 'attributes', '');
            return [
                'name' => $item->product->name . ' ' . $attributes,
                'price' => $item->price,
        ...
        $token = setting('shipping_ghtk_api_token_key') ?: '';
        $orderBody = json_encode((object)["products" => $products, "order" => (object) $orderData]);
        $curl = curl_init();
        ...
        if ($response->success) {
            ...
            $result->setMessage("Các đơn hàng đã được add vào hệ thống GHTK thành công");

            Arr::set($shipment, 'shipment_id', $response->order->label);
            Arr::set($shipment, 'tracking_id', $response->order->tracking_id);
            Arr::set($shipment, 'estimate_date_shipped', explode(' ', $response->order->estimated_deliver_time)[1]);
            Arr::set($shipment, 'status', ghtk_match_status_id($response->order->status_id));
            ...
            if (!$result->isError()) {
                ...
                OrderHistory::query()->create([
                    ...
                    'description' => trans('plugins/ecommerce::order.by_username'),
                    'order_id' => $order->getKey(),
                    'user_id' => Auth::id(),
                ]);
                ...
                ShipmentHistory::query()->create([
                    ...
                    'description' => Auth::user()->name . ' đã tạo 1 vận đơn từ đơn hàng ' . $order->code,
                    'shipment_id' => $shipment->id,
                    'order_id' => $order->getKey(),
                    'user_id' => Auth::id(),
                ]);
            }
        } else $result->setError()->setMessage($response->message);
