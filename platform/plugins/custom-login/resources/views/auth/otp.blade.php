    ...
    <div class="row">
        <div class="col-12">
            <div class="phone">
                ...
                <h6 style="margin-bottom: 30px;">Vui lòng nhập mã OTP được gửi trong app WGHN<br> để xác minh tài khoản của bạn
                </h6>
                <span>Mã OTP đã gửi đến app WGHN</small>
                <form action="{{ route('access.login.post') }}" method="post" id="otp-form" class="otp-form">
                    @csrf
                    <input type="hidden" name="phone" value="{{ $phoneSendToApp }}">
                    <div class="otp-field" id="otp-field">
                    </div>
                    <div id="error-msg" class="hide login-mgs"></div>
                    <div class="buttons">
                        <button id="btn_prev" type="button"
                            onclick="window.location.href= localStorage.getItem('lgtg') ?? '{{ route('access.login') }}';">Quay
                            lại</button>
                        <button id="btn_next" type="submit">Tiếp tục</button>
                    </div>
                </form>
                <div class="fw-normal text-muted mb-2">
                    Bạn chưa nhận được mã? <a
                        href="{{ route('access.login', ['phonenumber' => $phone, 'countryCode' => $countryCode]) }}"
                        class="text-primary fw-bold text-decoration-none">Gửi lại</a>
                </div>
            </div>
        </div>
    </div>
