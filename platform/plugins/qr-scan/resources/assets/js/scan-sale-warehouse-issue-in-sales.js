...
const renderBatchProductInfo = (data) => {
    $('#product_batch_table_list').html(data?.map((item, index) => {
        let arrAttribute = item?.product?.variation_product_attributes || [];
        let color = '';
        let size = '';
        if(arrAttribute.length > 0)
        {
            arrAttribute.forEach(value => {
                if(value?.attribute_set_slug == "size"){
                    size = value.title;
                }else if(value?.attribute_set_slug == "color"){
                    color = value.title;
                }
            })
        }
...
const handleResponseScanProduct = (res, code) => {
    ...
    const index = products?.findIndex((item =>
        ...
        && SALE_WAREHOUSE_CHILD_MODEL == d?.warehouse_type
    ));
...
document.addEventListener("DOMContentLoaded", (event) => {
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        ...
                        if (window.warehouse_id == res.data?.warehouse_id
                            && SALE_WAREHOUSE_CHILD_MODEL == res?.data?.warehouse_type) {
    ...
    $(document).on('click', '#save_qr_scan_btn', function () {
        const baseUrl = route('sale-issue.confirm-receipt-in-tour');
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                issue_id: $('#save_qr_scan_btn').data('id'),
                products: dataSend ?? '',
            },
