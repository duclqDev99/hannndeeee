...
const _selfModal = '#scanBatchModal';
...
const renderBatches = (data) => {
    $(`${_selfModal} #batch_table_list`).html(data?.map((item, index) => {
        ...
        <td> ${item?.product_in_batch?.length || 0} </td>
        <td >
        ${item.is_scan
...
const handleScanned = (res) => {
    ...
    if (qr.status.value == 'pendingstock') {
        ...
        if (batchIndex > -1) {
            qrBatchScanned.push({ qr_id: qr.id, batch_id: qr.reference.id })
...
document.addEventListener("DOMContentLoaded", (event) => {
    batches = window.batches.map(item => ({ ...item, is_scan: false }));
    console.log(window.warehouse)
    renderBatches(batches);

    $("body")
        .on('shown.bs.modal', '#scanBatchModal', () => Scan.start())
        .on('hide.bs.modal', '#scanBatchModal', () => Scan.stop());
    ...
    $("body").on('click', `${_selfModal} #save_qr_scan_btn`, function () {
        let qrcode_parent_id = [];
        qrcode_parent_id['data'] = qrBatchScanned;
        qrcode_parent_id['scan_batch_warehouse'] = true;

        createQRForBatch($(this), [], qrcode_parent_id, 'left');
