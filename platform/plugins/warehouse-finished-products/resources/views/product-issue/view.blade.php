...
@section('content')
    ...
    @php
        $strCenter = '';
        if (isset($actualIssue) || empty($actualIssue)) {
                                        ...
                                        <div class="card-header flexbox-grid-default">
                                            <div class="w-75">
                                                <label $class="title-product-main text-no-bold">{{ __('Phiếu xuất kho') }}
                                                    <strong>{{ $productIssue->proposal_code }}</strong> -
                                                    Xuất từ: <strong>{{ $productIssue->warehouse_name }}

                                                        @if ($productIssue->is_warehouse == 2)
                                                            : Xuất lẻ
                                                        @else
                                                ...
                                                <div>
                                                    Mã đơn hàng: <strong>{{ $productIssue->general_order_code }}</strong>
                                                </div>
                                                <div>
                                                    Người duyệt đề xuất:
                                                    ...
                                                    @foreach ($productIssue->productIssueDetail as $orderProduct)
                                                        <tr class="item__product">
                                                            ...
                                                            <td class="pl5 p-r5" width="20%">
                                                                ...
                                                                <div>
                                                                    @php
                                                                        echo $productIssue->is_batch !== 0 ? '' : '(Màu: ' . $orderProduct->color . ' Size: ' . $orderProduct->size . ' )';
                                                                    @endphp
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td class="pl5 p-r5  text-end" width="15%">
                                                                Số lượng: <span
                                                                    class="check__highlight">{{ $orderProduct->quantity }} lô</span>
                                                            </td>
                                                        ...
                                                        @php
                                                            ...
                                                            $totalPriceReceipt += $orderProduct->price * $orderProduct->quantity;
                                                        @endphp
                                                    @endforeach
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="myForm" action="{{ route('product-issue.issue.export', $productIssue->id) }}"
                                method="POST">
                                @csrf
