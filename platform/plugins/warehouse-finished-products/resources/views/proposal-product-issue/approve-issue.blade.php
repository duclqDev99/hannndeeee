...
@section('content')
            ...
            <div class="flexbox-layout-sections">
                <div class="flexbox-layout-section-primary mt20">
                    <div class="card ui-layout__item">
                            ...
                            <div class="pd-all-20">
                                <div class="card-header justify-content-between">
                                    <div class="flexbox-auto-right mr5">
                                        <label $class="title-product-main text-no-bold">{{ __('Duyệt đơn đề xuất xuất') }}
                                            từ: <strong>{{ $proposal->warehouse_name }} <i
                                                    class="fa-solid fa-arrow-right"></i>

                                                @if ($proposal->warehouse_type != '')
                                        ...
                                        <div>
                                            Mã phiếu: <strong>{{ $proposal->proposal_code }}</strong>
                                        </div>
                                        <div>
                                        ...
                                        <tbody>
                                            @foreach ($proposal->proposalProductIssueDetail as $orderProduct)
                                                <tr class="item__product">
                                                    <td class="width-60-px min-width-60-px vertical-align-t text-center"
                                                        width="20%">
                                                        <div class="wrap-img">
                                                        ...
                                                        <span>
                                                            ...
                                                            @php
                                                                echo $orderProduct->size !== '' || $orderProduct->color !== '' ? ' - (Màu: ' . $orderProduct->color . '- Size: ' . $orderProduct->size . ' ) ' : '';
                                                            @endphp
                                                    ...
                                                    <td class="pl5 p-r5 text-center" width="20%">
                                                        {{ $orderProduct->color == '' || $orderProduct->size == '' ? 'Xuất lô' : 'Xuất lẻ' }}
                                                    </td>
                                                    ...
                                                    <td class="pl5 p-r5 text-center" width="25%">
                                                        ...
                                                        <span style="white-space: nowrap;">Tồn kho:
                                                            {{ isset($quantity) ? $quantity : 0 }}</span>
                                                    </td>
                                                    <td class="pl5 p-r5 text-end">
                                                        <div class="d-flex justify-content-center">
                                                            ...
                                                            @else
                                                                Số lượng duyệt:
                                                                <div
                                                                    class="{{ $orderProduct->quantityExamine > $quantity ? 'text-danger' : '' }}">
                                                                    {{ $orderProduct->quantityExamine }}
                                                                    {{ $orderProduct->color == '' || $orderProduct->size == '' ? 'Lô' : 'SP' }}
                                                                </div>
                                                            @endif
                                        ...
                                        <div class="row">
                                            <div class="col-lg-6 col-sm-12">
                                                <div class="py-3 w-100">
                                                    ...
                                                    @else
                                                        <textarea class="form-control textarea-auto-height" name="description" rows="4" disabled
                                                            placeholder="{{ __('Ghi chú') }}">{{ $proposal->description_examine }}</textarea>
                                                    @endif
                                                </div>
                                            </div>
                                ...
                                <div class="mt10">
                                    @if ($proposal->status == ProposalIssueStatusEnum::PENDING)
                                        <button class="btn btn-primary" type="button"
                                            id="examineButton">{{ __('Duyệt đơn') }}</button>
                                        <button class="btn btn-danger" type="button"
                                            id="denyButton">{{ __('Từ chối') }}</button>
                                    @else
                                        <button class="btn btn-primary" type="button"
                                            id="approveButton">{{ __('Chuyển đơn đề xuất đến kho') }}</button>
                                    @endif
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form id="denyForm" method="post"
                                            action="{{ route('proposal-product-issue.denied', $proposal->id) }}">
                                            @csrf
                        ...
                        <div class="modal" id="examineModal" tabindex="-1" role="dialog">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-center py-4">
                                        ...
                                        <h3>Xác nhận duyệt đơn đề xuất</h3>

                                    </div>
                                    <form id="examine" method="post"
                                        action="{{ route('proposal-product-issue.examine', $proposal) }}">
                                        @csrf
                                        <input type="hidden" name="dataProduct" id="dataProduct">
                                        <input type="hidden" name="expectDate" id="expectDate" />
                                        <input type="hidden" name="descriptionForm" id="descriptionForm" />
                                        <div class="modal-body d-flex justify-content-end">
                                            <button type="button" class="btn btn-secondary"
                                                data-bs-dismiss="modal">Đóng</button>
                                            <button type="submit" class="btn btn-success ms-2" id="examineSubmit">Xác
                                                nhận</button>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-center py-4">
                                        ...
                                        <h3>Chuyển đơn đề xuất đến kho</h3>

                                    </div>
                                    <form id="denyForm" method="post"
                                        action="{{ route('proposal-product-issue.approve', $proposal) }}">
                                        @csrf
                                        <div class="modal-body d-flex justify-content-end">
                                            <button type="button" class="btn btn-secondary"
                                                data-bs-dismiss="modal">Đóng</button>
                                            <button type="submit" class="btn btn-primary ms-2" id="approveSubmit">Xác
                                                nhận</button>
                                        </div>
                                    </form>
                                </div>
    ...
    <script>
        ...
        $("#examineButton").on("click", function() {
