        ...
        <div class="col-lg-6 col-12">
            ...
            <div class="ui-select-wrapper form-group">
                <select v-model="selectedWarehouseProduct" data-quantity="0" name="warehouse_product"
                    ...
                    <option v-for="warehouse in allWarehouse" :key="warehouse.id" :value="warehouse.id">{{
        warehouse.name }}
                    </option>
                </select>
            </div>
            ...
            <div class="flex-content">
                ...
                <div>
                    ...
                    <multi-select :products="filteredProducts" @select="onSelect" :placeholder="placeholderText"
                        :items="caseStates[radio].items" :radio="radio" :parent="true" :warehouse="true"
                        @update:products="handleUpdatedProducts" @search:products="searchProducts">
                    </multi-select>
                </div>
            </div>
                    ...
                    <tbody>
                        <tr v-for="item in caseStates[radio].items" :key="item.id">
                            ...
                            <td class="text-center">
                                <input type="hidden" :value="item.is_batch" :name="`product[${item.id}][is_batch]`">
                                ...
                                <input :name="`product[${item.id}][quantityStock]`" :value="item.quantityStock"
                                    hidden />
                            </td>
                            <td class="text-center">
...
export default {
    ...
    async mounted() {
        ...
        this.warehouseReceipt = $('#warehouse_id').val();
    ...
    methods: {
        ...
        submitForm(e) {
            ...
            const form = document.getElementById('botble-warehouse-forms-proposal-receipt-products-form');
            form.submit();
        ...
        getProductInFormProposal(idProposal) {
            $("form button[type='submit']").prop('disabled', true);
            axios.get(route('proposal-receipt-products.getProposal', { id: idProposal }))
                .then(res => {
                    console.log(res);
                    ...
                    if (data.is_warehouse == "inventory") {
                        console.log(1);
                        this.radio = 3
                        this.inventory = false;
                        this.isWarehouse = false
                        this.fetchAllProduct(data.proposal_detail);
                    }
                    else {
                        this.radio = 0
                        const newState = this.caseStates[this.radio];
        ...
        async fetchData(data, newState) {
            ...
            await this.getProductByWarehouse('Botble\\WarehouseFinishedProducts\\Models\\QuantityProductInStock', data.proposal.warehouse_id, (dataList) => {
        ...
        fetchAllProduct(data = null) {
            ...
            axios.get(route('finished-products.getAllListProduct', { id: this.warehouseReceipt, search: this.keySearch, type: 'warehouse-finished' }))
                .then(response => {
                    ...
                    if (data) {
                        data.map(item => {
                            ...
                            const dataInsert = {
                                ...
                                'quantityStock': 0,
                                'is_batch': 0,
                                'image': item.prd.parent_product[0].image
