...
export default {
    ...
    methods: {
        ...
        getProductInFormProposal(idProposal) {
            $("form button[type='submit']").prop('disabled', true);
            axios.get(route('proposal-hub-receipt.proposal', { proposal_id: idProposal }))
                .then(res => {
                    const data = res.data
        ...
        fetchAllProduct(data = null) {
            ...
            axios.get(route('finished-products.getAllListProduct', { id: this.warehouseReceipt, search: this.keySearch }))
                .then(response => {
                    ...
                    if (data) {
                        data.map(item => {
                            const foundItem = newState.dataList.find(data => data.id === item.product_id);
                            if (foundItem) {
                                foundItem.quantity = item.quantity;
                                newState.items.push(foundItem);
                            }
                            else {
                                const dataInsert = {
                                    ...item,
