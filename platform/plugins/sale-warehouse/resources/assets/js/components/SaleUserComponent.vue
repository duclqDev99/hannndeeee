      ...
      <div class="flexbox">
        <div class="flex-content">
          <div class="button-group" style="margin-bottom: 5px">
            <button type="button" @click="reset" class="btn btn-secondary" style="background-color:#46C2CB; color: #fff; margin-right:5px"
            v-if="items.length > 0">Xóa tất cả</button>
          </div>
          <div>
            <multi-list-select
              :list="dataList"
        ...
        <div class="flex-result">
          <table class="ui celled table">
            ...
            <tr>
              <td>{{item.name}}</td>
              <input type="hidden" name="sale_warehouse_id[]" :value="item.id">
            </tr>
            </tbody>
          </table>
        </div>
