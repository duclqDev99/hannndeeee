...
class PurchaseOrderTable extends TableAbstract
{
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('confirm_by', function (Order $item) {
                $created = get_action(\Botble\OrderStepSetting\Enums\ActionEnum::HGF_ADMIN_CONFIRM_ORDER, $item->id);
                if ($created) {
            ...
            ->editColumn('status', function (Order $item) {
                return '<span class="text-wrap" style="max-width:100px">' . $item->lastAction->actionSetting->title . '</span>';
            })
            ->editColumn('operators', function (Order $item) {
                return view('plugins/order-hgf::table-columns.dropdown-actions', compact('item'));
            });
    ...
    public function query(): Relation|Builder|QueryBuilder
    {
        ...
        $query = $this
            ...
            ->whereHas('steps', function ($q) use ($department_code) {
    ...
    {
        return [];
        return $this->addCreateButton(route('retail.sale.purchase-order.create'), 'purchase-order.create');
    }
