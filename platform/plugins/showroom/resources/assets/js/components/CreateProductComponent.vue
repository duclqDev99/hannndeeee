    ...
    methods: {
            ...
            try {
                ...
                let filteredChildProducts = this.child_products.map(item => {
                    ...
                    return {
                        ...
                        showroom_id: this.agentWarehouse['showroom_id'],
                        product_id: item.id,
                        select_qty: item.select_qty,
                        showroom_product_id: agencyProduct ? agencyProduct.id : null,
                        status: 'created',
                        name: item.name,
                        variation_attributes: item.variation_attributes,
                    };
                });
                const response = await axios.post(route('showroom-warehouse.store-product-manual'),{
                    products: filteredChildProducts,
                });
                window.location.href = route('showroom-warehouse.detail-odd', this.agentWarehouse['id']);
        ...
        fillterProductQuantity: function(id){
            if(typeof this.agencyProduct == 'object'){
                ...
                return item ? item.quantity_qrcode + item.quantity_not_qrcode : 0;
            }
            return 0
        }
