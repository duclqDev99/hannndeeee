        ...
        <div class="flex-content">
          <div class="button-group" style="margin-bottom: 5px">
            <button
              ...
              style="background-color:#46C2CB; color: #fff; margin-right:5px"
              v-if="items.length > 0"
            >
              Xóa tất cả
            </button>
          ...
          <table class="ui celled table">
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id">
                <td>{{ item.name }}</td>
                <input type="hidden" name="showRoom_id[]" :value="item.id" />
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
    ...
    data() {
        const dataAttribute = this.data ? this.data.choices || [] : [];
        return {
