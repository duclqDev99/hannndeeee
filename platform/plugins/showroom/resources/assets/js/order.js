class OrderAdminManagement {
    init() {
        ...
        // $(document).find('#confirm-payment-modal').on('hidden.bs.modal', (event) => {
        //     $('#confirm-payment-modal .modal-body').html('');
        // })

        $(document).on('click', '.btn-trigger-confirm-payment-qr', (event) => {
            event.preventDefault()
            $('#qrcode').attr('src', '');
            let modalQR = document.querySelector('#confirm-payment-modal .modal-body');
            window.insertSvgLoading(modalQR)
            const qrChannel = new BroadcastChannel('QR-customer-channel');
            $.ajax({
                url: $('.btn-trigger-confirm-payment-qr').attr('data-url'),
                type: 'POST',
                dataType: 'json',
                success: function(res) {
                    const loaderFrame = $(modalQR).find('.loader-container')
                    if (loaderFrame) {
                        loaderFrame.remove();
                    }
                    let data = res.data;
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    $('#your-form-field').val('test');
                }
            });
            $('#confirm-payment-modal').modal('show')
        })

        $(document).on('click', '.btn-trigger-confirm-payment-bank-transfer', (event) => {
            event.preventDefault()

            $('#confirm-payment-bank-transfer-accept-modal').modal('show')
        })

        $(document).on('click', '.btn-trigger-confirm-payment-cash', (event) => {
            event.preventDefault()
            $('#confirm-payment-cash-order-button').data('target', $(event.currentTarget).data('target'))
            $('#confirm-payment-cash-modal').modal('show')
        })

        $(document).on('click', '#confirm-payment-cash-order-button', (event) => {
            event.preventDefault()
            const _self = $(event.currentTarget)
            ...
            $httpClient.make()
                ...
                .then(({ data }) => {
                    if (!data.error) {
                        Botble.showSuccess(data.message)
                        $('#main-order-content').load(`${window.location.href} #main-order-content > *`)
                        $('#confirm-payment-cash-modal').modal('hide')
                    } else {
                        Botble.showError(data.message)
                    }
                })
        ...
        $(document).on('click', '.download_img', (event) => {
            event.preventDefault()
