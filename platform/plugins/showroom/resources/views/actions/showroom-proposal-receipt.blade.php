@php
    ...
    if ($item->status == ProposalAgentEnum::PENDING) {
        ...
        // if (Auth::user()->hasPermission('proposal-showroom-receipt.approve') || Auth::user()->hasPermission('showroom.all')) {
        //     $actionApprove =
        //         '
        //                 <li> <a data-bs-toggle="tooltip" class="dropdown-item text-success" href="' .
        //         route('proposal-showroom-receipt.approve', $item) .
    ...
    if ($actionApprove == '') {
        $actionView =
            '
                    ...
                    href="' .
            route('proposal-showroom-receipt.view', $item->id) .
    ...
    $editView = '';
    if (($item->status == ProposalAgentEnum::PENDING || $item->status == ProposalAgentEnum::DENIED) && $item->issuer_id == Auth::user()->id) {
        $editView =
            '
                    <li><a data-bs-toggle="tooltip"     class="dropdown-item text-primary" data-bs-original-title="Edit" href="' .
            route('proposal-showroom-receipt.edit', $item->id) .
