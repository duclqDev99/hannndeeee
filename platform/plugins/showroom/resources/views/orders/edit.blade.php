        ...
        <div class="row row-cards">
            <div class="col-md-9">
                <x-core::card class="mb-3">
                    ...
                    @if (!$order->is_scanned)
                        <div class="card-header d-flex justify-content-between">
                            <p class="mb-0">
                                ...
                                Vui lòng quét QR sản phẩm có thông tin bên dưới để chuẩn bị hàng giao cho đơn vị vận chuyển
                            </p>
                        </div>
                    @endif
                    ...
                    <x-core::table :hover="false" :striped="false">
                        <x-core::table.body>
                            <script>
                                window.orderProducts = @json($order->products);
                            </script>
                                ...
                                <x-core::table.body.row>
                                    ...
                                    <x-core::table.body.cell style="width: 45%">
                                        <div class="d-flex align-items-center flex-wrap">
                                            <a href="{{ $product && $product->id && Auth::user()->hasPermission('products.edit') ? route('products.edit', $product->id) : '#' }}"
                                            ...
                                            <ul class="list-unstyled ms-1 small">
                                                ...
                                                @if (is_plugin_active('marketplace') && $order->store->name)
                                                    <li class="ws-nm">
                                                        ...
                                                        <a class="fw-semibold text-decoration-underline"
                        ...
                        <div class="row">
                            <div class="col-md-6 offset-md-6">
                                    ...
                                    <x-core::table.body>
                                        ...
                                        <x-core::table.body.row>
                                            <x-core::table.body.cell>
                                                ...
                                                <span class="small d-block">{{ $weight }}
                                        ...
                                        @if($order->payment->payment_channel == PaymentMethodEnum::BANK_TRANSFER)
                                            <x-core::table.body.row>
                                                <x-core::table.body.cell>
                                                        <span>Nội dung chuyển khoản</span>
                                                </x-core::table.body.cell>
                                                <x-core::table.body.cell>
                                                    {{ $order->payment->content_banking}}
                                                </x-core::table.body.cell>
                                            </x-core::table.body.row>
                                            <x-core::table.body.row>
                                                <x-core::table.body.cell>
                                                    @foreach($order->payment->images as $image)
                                                        <img src="{{ RvMedia::getImageUrl($image,'thumb', false) }}" data-full="{{ RvMedia::getImageUrl($image,'', false) }}" alt="Hình ảnh chuyển khoản" class="thumbnail-img">
                                                    @endforeach
                                                </x-core::table.body.cell>
                                            </x-core::table.body.row>
                                ...
                                <div class="btn-list justify-content-end my-3">
                                    <x-core::button tag="a"
                                        href="{{ route('showroom.orders.print-showroom-order', $order->id) }}?type=print"
                                        target="_blank" icon="ti ti-printer">
                                        {{ trans('plugins/showroom::order.print_order') }}
                                    </x-core::button>
                                    <x-core::button tag="a"
                                        href="{{ route('showroom.orders.print-showroom-order-vat', $order->id) }}?type=print"
                                        target="_blank" icon="ti ti-upload">
                                        {{ trans('plugins/showroom::order.export_bill_vat') }}
                                    </x-core::button>
                                </div>

                                <form action="{{ route('orders.edit', $order->id) }}">
                                    <x-core::form.textarea :label="trans('plugins/ecommerce::order.note')" name="description" :placeholder="trans('plugins/ecommerce::order.add_note')"
                                        :value="$order->description" class="textarea-auto-height" />
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        @if (
                            $order->status == Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED ||
                                (is_plugin_active('payment') && $order->payment->id))
                            <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                                ...
                                @elseif (is_plugin_active('payment') && $order->payment->id)
                                    ...
                                    @if (!$order->is_from_home)
                                        <div class="btn-list">
                                            @if ((!$order->payment->status || in_array($order->payment->status, [Botble\Payment\Enums\PaymentStatusEnum::PENDING]))  && $order->payment->payment_channel == PaymentMethodEnum::BANK_TRANSFER)
                                                <x-core::button type="button" color="info" class="btn-trigger-confirm-payment-bank-transfer"
                                                    :data-target="route('showroom.orders.confirm-payment', $order->id)" :data-url="route(
                                                        'order-transactions.send-request-payment',
                                                        $order->id,
                                                    )">
                                                   {{ trans('plugins/ecommerce::order.confirm_payment') }}
                                                </x-core::button>
                                                <x-core::button type="button" color="info" class="download_img"
                                                    :data-target="route('showroom.orders.confirm-payment', $order->id)" :data-url="route(
                                                        'order-transactions.send-request-payment',
                                                        $order->id,
                                                    )">
                                                    {{ trans('plugins/showroom::order.download_qr_image') }}
                                                </x-core::button>
                                                <x-core::button type="button" color="info"
                                                    class="btn-trigger-confirm-payment-qr" :data-target="route('showroom.orders.confirm-payment', $order->id)"
                                                    :data-url="route(
                                                        'order-transactions.send-request-payment',
                                                        $order->id,
                                                    )">
                                                    {{ trans('plugins/showroom::order.create_qr_image') }}
                                                </x-core::button>
                                            @elseif(in_array($order->payment->status, [Botble\Payment\Enums\PaymentStatusEnum::PENDING]) && $order->payment->payment_channel == PaymentMethodEnum::CASH)
                                                <x-core::button type="button" color="info"
                                                   class="btn-trigger-confirm-payment-cash" :data-target="route('showroom.orders.confirm-payment', $order->id)">
                                                    {{ trans('plugins/ecommerce::order.confirm_payment') }}
                                                </x-core::button>
                                                <x-core::modal.action
                                                    id="confirm-payment-cash-modal"
                                                    type="info"
                                                    ...
                                                    :submit-button-attrs="['id' => 'confirm-payment-cash-order-button']"
                        ...
                        @if ($order->is_confirmed && $order->payment->customer_type == \Botble\Ecommerce\Models\Customer::class)
                            <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                                @if ($order->is_scanned)
                                    @switch($order->status->getValue())
                                        @case('delivery_failed')
                                            <div class="text-uppercase">
                                                <x-core::icon name="fa-solid fa-exclamation" @class(['text-warning' => $order->is_scanned]) />
                                                Chưa hoàn sản phẩm
                                            </div>
                                            <button class="btn btn-info btn-return-product" data-bs-toggle="modal"
                                                data-showroom-order-id="{{ $showroomOrder->id }}"
                                                data-bs-target="#return-product-modal">
                                                Hoàn sản phẩm
                                            </button>
                                        @break
                                        ...
                                        @case('returned')
                                            <div class="text-uppercase">
                                                <x-core::icon name="ti ti-check" @class(['text-success' => $order->is_scanned]) />
                                                Đã hoàn sản phẩm
                                            </div>
                                        @break
                                ...
                                @else
                                    <div class="text-uppercase">
                                        <x-core::icon name="fa-solid fa-qrcode" @class(['text-secondary' => !$order->is_scanned]) />
                                        Chưa cập nhật sản phẩm
                                    </div>
                                    @if ($order->status->getValue() == 'processing')
                                        <button class="btn btn-info" data-bs-toggle="modal"
                                            data-bs-target="#scanProductToOrderShowroom">
                                            Quét QR
                                        </button>
                                    @endif
                                @endif
                            ...
                            {{--  --}}
                            @if (
                                $order->payment->payment_channel == 'cod' ||
                                    ($order->payment->payment_channel == 'bank_transfer' && $order->payment->status->getValue() == 'completed'))
                                <div class="p-3 d-flex justify-content-between align-items-center">
                                    ...
                                    @else
                                        @if ($order->shipment->id)
                                            ...
                                            <a class="btn btn-print-shipment" data-id="{{ $order->shipment->id }}">
                                        ...
                                        @else
                                            ...
                                            <x-core::button type="button" class="btn-trigger-shipment" color="info"
                                ...
                                <li @class(['step-item', 'user-action' => $history->user_id])>
                                    <div class="h4 m-0">
                                        @if (in_array($history->action, ['confirm_payment', 'refund']))
                                            <a class="show-timeline-dropdown text-primary"
                                    ...
                                    <div class="text-secondary">{{ BaseHelper::formatDateTime($history->created_at) }}
                                    </div>
                                    @if ($history->action == 'refund' && Arr::get($history->extras, 'amount', 0) > 0)
                                        <div class="timeline-dropdown bg-body mt-2 rounded-2" style="display: none"
                                    ...
                                    @if ($history->action == 'send_order_confirmation_email')
                                        <x-core::button type="button" color="primary" :outlined="true"
            ...
            <div class="col-md-3">
                ...
                <x-core::card>
                        ...
                        <div class="p-3">
                            <div class="mb-3">
                                <span class="avatar avatar-lg avatar-rounded"
                                    style="background-image: url('{{ $order->user->id ? $order->user->avatar_url : $order->address->avatar_url }}')"></span>
                            </div>
                            ...
                            @if (EcommerceHelper::countDigitalProducts($order->products) != $order->products->count())
                                ...
                                <div class="p-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        @if ($order->status != Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                                            <a class="btn-trigger-update-shipping-address btn-action text-decoration-none"
                                                href="#" data-placement="top" data-bs-toggle="tooltip"
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    @if ($order->status !== Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                                        <div class="flexbox-auto-content-right text-end">
                                            <a class="btn-trigger-update-tax-information" href="#">
                    ...
                    <x-core::card.footer>
                        <div class="btn-list">
                            <x-core::button tag="a" :href="route('showroom.orders.create')">
                                {{ trans('plugins/ecommerce::order.create_new_order') }}
                            </x-core::button>
                            @if (!$order->payment->status || in_array($order->payment->status, [Botble\Payment\Enums\PaymentStatusEnum::PENDING]))
                                <x-core::button type="button" :data-target="route('showroom.orders.cancel', $order->id)" class="btn-trigger-cancel-order">
...
<x-core::modal.action id="resend-order-confirmation-email-modal" :title="trans('plugins/ecommerce::order.resend_order_confirmation')" :description="trans('plugins/ecommerce::order.resend_order_confirmation_description', [
    'email' => $order->user->email ?: $order->address->email,
])"
    :submit-button-attrs="['id' => 'confirm-resend-confirmation-email-button']" :submit-button-label="trans('plugins/ecommerce::order.send')" />

<x-core::modal.action id="cancel-shipment-modal" type="warning" :title="trans('plugins/ecommerce::order.cancel_shipping_confirmation')" :description="trans('plugins/ecommerce::order.cancel_shipping_confirmation_description')"
    :submit-button-attrs="['id' => 'confirm-cancel-shipment-button']" :submit-button-label="trans('plugins/ecommerce::order.confirm')" />

<x-core::modal id="update-shipping-address-modal" :title="trans('plugins/ecommerce::order.update_address')"
    button-id="confirm-update-shipping-address-button" :button-label="trans('plugins/ecommerce::order.update')" size="md">
...
@if ($order->taxInformation)
    <x-core::modal id="update-tax-information-modal" :title="trans('plugins/ecommerce::order.tax_info.update')"
        button-id="confirm-update-tax-information-button" :button-label="trans('plugins/ecommerce::order.update')" size="md">
...
<x-core::modal.action id="cancel-order-modal" type="warning" :title="trans('plugins/ecommerce::order.cancel_order_confirmation')" :description="trans('plugins/ecommerce::order.cancel_order_confirmation_description')" :submit-button-attrs="['id' => 'confirm-cancel-order-button']"
    :submit-button-label="trans('plugins/ecommerce::order.cancel_order')" />

@if (is_plugin_active('payment'))
    <x-core::modal id="confirm-payment-modal" type="info" title="Quét mã để thanh toán" size="md"
        data-order-id="{{ $order->id }}">
        <div class="text-start" style="display: flex; align-items: center; justify-content: center; height: 200px;">
            <img id="qrcode" height="100%" />
        </div>
    </x-core::modal>

    <x-core::modal id="confirm-refund-modal" :title="trans('plugins/ecommerce::order.refund')" button-id="confirm-refund-payment-button"
        size="lg">
        <x-slot:button-label>
            {{ trans('plugins/ecommerce::order.confirm_payment') }}
            <span
                class="refund-amount-text ms-1">{{ format_price($order->payment->amount - $order->payment->refunded_amount) }}</span>
        </x-slot:button-label>
    ...
    <x-core::modal title="Xác nhận chuyển khoản" id="confirm-payment-bank-transfer-accept-modal" data-url="{{route('showroom.orders.confirm-bank-transfer')}}" title="Title" button-id="confirm-payment-accept-button"
        size="lg">
        <div class="">
            <label for="content_banking">Nội dung chuyển khoản</label>
            <textarea name="content_banking" id="content_banking" class="form-control" rows="3"></textarea>
...
@include('plugins/qr-scan::scan-product-to-order-showroom')

{{-- <x-core::modal
id="return-product-modal"
:title="'Xác nhận hoàn trả hàng'"
button-id="confirm-return-product"
:button-label="'Xác nhận'"
>
@include('plugins/showroom::product.return')
</x-core::modal> --}}


<x-core::modal.action id="return-product-modal" :title="'Xác nhận hoàn trả hàng'" :description="'Tất cả sản phẩm thuộc đơn hàng sẽ được hoàn lại và tiếp tục bán'" :submit-button-attrs="['id' => 'confirm-return-product', 'data-showroom-order-id' => $showroomOrder->id]"
    :submit-button-label="'Xác nhận'" />
@endpushif

@push('footer')
    <script>
        document.addEventListener("DOMContentLoaded", (event) => {
            $(document).on('click', '.btn-print-shipment', function(event) {
                $('body iframe').remove();
                const shipment_id = $(this).data('id');

                $.ajax({
                    url: '/admin/showroom-shipments/print',
                    data: {
                        shipment_id
                    },
                    method: 'POST',
                    ...
                    success: (response, status, xhr) => {
                        ...
                        iframe.style.display = 'none'; // Ẩn iframe để không hiển thị trên trang
                        iframe.src = blobURL;
            ...
            $(document).on('click','#confirm-payment-accept-button', function(event){
                let modal = $('#confirm-payment-bank-transfer-accept-modal');
                let images = modal.find('input[name="images[]"]').map(function() {
                    if($(this).val()){
                        return $(this).val();
                    }
                }).get();
                ...
                let content_banking = modal.find('textarea[name="content_banking"]').val();
                $.ajax({
                    url: modal.data('url'),
                    method: 'POST',
                    data: {
                        'images' : images,
                        'order_id' : {{$order->id}},
                        'content_banking' : content_banking
                    },
