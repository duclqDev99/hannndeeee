...
class ShowroomProductOddTable extends TableAbstract
{
    ...
    public function query(): Relation|Builder|QueryBuilder
    {
        if (!Auth::user()->hasPermission('showroom.all')) {
            $userShowroom = get_showroom_for_user();

            $listWarehouse = $userShowroom->reduce(function ($carry, $showroom) {
                return $carry->merge($showroom->warehouses->pluck('id'));
            }, collect());

            $listProduct = ShowroomProduct::whereIn('warehouse_id', $listWarehouse)->pluck('product_id');
        } else {
            $listProduct = ShowroomProduct::pluck('product_id');
        }
        ...
        $query = $this
            ...
            ->with([
                'variations' => function ($query) use ($listProduct) {
        ...
        return <<<JS
            ...
            $(document).on('click', '.show_collapse_btn', function(){
                ...
                if(!_catch && !$(this).closest('tr').next().find('.collapse').hasClass('show')){
                    ...
                    $.ajax({
                        method: 'GET',
                        url: route('showroom-product.detail', { id: product_id }),
                        contentType: 'html',
                        success: res => {
