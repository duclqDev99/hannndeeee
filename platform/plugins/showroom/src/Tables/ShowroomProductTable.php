...
class ShowroomProductTable extends TableAbstract
{

    public function setup(): void
    {
        $this
            ->model(ShowroomProduct::class)
            ->addActions([
                EditAction::make()
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ->eloquent($this->query())
            ->editColumn('name', function (ShowroomProduct $item) {
                return $item?->warehouse?->name . '-' . $item?->warehouse?->showroom?->name;
            })

            ->editColumn('product.name', function (ShowroomProduct $item) {
            ...
            ->editColumn('product.sku', function (ShowroomProduct $item) {

                return $item?->product?->sku;
            })
            ->editColumn('image', function (ShowroomProduct $item) {
                $imageTag = '';
