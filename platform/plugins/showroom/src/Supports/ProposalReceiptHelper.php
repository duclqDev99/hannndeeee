...
{
    public function makeMaterialPDF(ShowroomProposalReceipt $invoice): PDFHelper|Dompdf
    {
    ...
    public function downloadInvoice(ShowroomProposalReceipt $invoice): Response
    {
        return $this->makeMaterialPDF($invoice)->download(sprintf('material-%s.pdf', $invoice->proposal_code));
    }

    public function streamInvoice(ShowroomProposalReceipt $invoice): Response
    {
    ...
    protected function getDataForInvoiceTemplate(ShowroomProposalReceipt $invoice): array
    {
        ...
        $data = [
            ...
            'warehouse_parent_name' => $invoice->warehouseReceipt?->showroom?->name ?? '',
            'warehouse_parent_address' => $invoice->warehouseReceipt?->showroom?->address ?? ''
        ];

        return apply_filters('ecommerce_invoice_variables', $data, $invoice);
    }

    public function getDataForPreview(): ShowroomProposalReceipt
    {
        $matial = new ShowroomProposalReceipt([
            'id' => 'INV-1',
