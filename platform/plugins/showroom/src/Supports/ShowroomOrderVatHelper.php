...
class ShowroomOrderVatHelper
{
    ...
    {
        $defaultPath = platform_path('plugins/showroom/resources/views/orders/pdf/order-vat.tpl');
        $storagePath = storage_path('app/templates/order-vat.tpl');
    ...
    protected function getDataForInvoiceTemplate(ShowroomOrderViewEc $invoice): array
    {
        ...
        foreach ($invoice->products as $key => $value) {
            ...
            $dataProduct[$value->product_id]['sub_total'] = ($value->price + $value->tax_amount) * $value->qty;
            $dataProduct[$value->product_id]['sub_total_num'] = number_format(($value->price + $value->tax_amount) * $value->qty);
        ...
        $data = [
            ...
            'total_amount_string' => ucfirst(convert_number_to_words(intval($totalAmount))),
            ...
            'phone_customer' => $invoice->user?->phone ?? '0',
