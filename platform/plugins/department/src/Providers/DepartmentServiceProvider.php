<?php

namespace Botble\Department\Providers;

use Botble\Department\Models\Department;
use Botble\Base\Facades\DashboardMenu;
use Botble\Base\Traits\LoadAndPublishDataTrait;
use Botble\Base\Supports\ServiceProvider;
use Illuminate\Routing\Events\RouteMatched;

class DepartmentServiceProvider extends ServiceProvider
{
    use LoadAndPublishDataTrait;

    public function boot(): void
    {
        $this
            ->setNamespace('plugins/department')
            ->loadHelpers()
            ->loadAndPublishConfigurations(['permissions'])
            ->loadMigrations()
            ->loadAndPublishTranslations()
            ->loadAndPublishViews()
            ->loadRoutes();

        // if (defined('LANGUAGE_ADVANCED_MODULE_SCREEN_NAME')) {
        //     \Botble\LanguageAdvanced\Supports\LanguageAdvancedManager::registerModule(Department::class, [
        //         'name',
        //     ]);
        // }

        $this->app['events']->listen(RouteMatched::class, function () {
            DashboardMenu::registerItem([
                'id' => 'cms-plugins-department',
                'priority' => 5,
                'parent_id' => null,
                'name' => 'Quản lý bộ phận',
                'icon' => 'fa fa-list',
                'url' => route('department.index'),
                'permissions' => ['department.index'],
            ]);
            DashboardMenu::registerItem([
                'id' => 'cms-plugins-department-list',
                'priority' => 5,
                'parent_id' => 'cms-plugins-department',
                'name' => 'Danh sách bộ phận',
                'icon' => 'fa fa-list',
                'url' => route('department.index'),
                'permissions' => ['department.index'],
            ]);
            DashboardMenu::registerItem([
                'id' => 'cms-plugins-department-user',
                'priority' => 5,
                'parent_id' => 'cms-plugins-department',
                'name' => 'Nhân viên bộ phận',
                'icon' => 'fa-solid fa-user-group',
                'url' => route('department-user.index'),
                'permissions' => ['department.index'],
            ]);
        });
    }
}
