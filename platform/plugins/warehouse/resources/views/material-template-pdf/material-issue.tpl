...
<body>
    <table class="invoice-info-container">
        <tr>
            ...
            <td style="text-align: right;">
                ...
                <p style="text-align: center;">( Ban hành theo Thông tư số 200/2014/TTBTC ngày 22/12/2014 của Bộ Tài chính )</p>
            </td>
        </tr>
        <tr>
            <td class="info__company" style="text-align: left;">
                {% if company_name %}
                <h4>{{ company_name }}</h4>
                {% endif %}
                {% if company_address %}
                ...
                {% endif %}
                {% if company_tax_id %}
                <p>Mã số thuế: {{ 'plugins/ecommerce::ecommerce.setting.tax_id'|trans }}: {{ company_tax_id }}</p>
                {% endif %}
                {% if company_phone %}
    ...
    <table>
        ...
        <tr>
            <td class="" colspan="2">
            - Lý do xuất kho: {{ invoice['description'] }}				
            </td>
        </tr>
        <tr>
            <td class="">
                - Xuất tại kho (ngăn lô): <span class="text-bold"> {{ invoice['warehouse_name'] }}	</span>			
            </td>
            <td class="">
                - Địa điểm: <span class="text-bold"> {{invoice['warehouse_address']}}</span>			
            </td>
        </tr>
    </table>
    <table id="table_material" class="line-items-container table-bordered">
        ...
        <tr>
            ...
            <th rowspan="2" class="heading-description center">Mã số</th>
            <th rowspan="2" class="heading-description center">Đơn vị tính</th>
            <th colspan="2" class="heading-quantity center">Số lượng</th>
            <th rowspan="2" class="heading-price center">Đơn giá</th>
            <th rowspan="2" class="heading-subtotal center">Thành tiền</th>
        </tr>
        <tr>
            <th class="heading-quantity center">Yêu cầu</th>
        ...
        {% if proposal_detail %}
            {% for index,val in proposal_detail %}
                <tr>
                    ...
                    <td class="center">{{val['material_code']}}</td>
                    <td class="center">{{val['material_unit']}}</td>
                    <td class="center">{{val['material_quantity']}}</td>
                    <td class="center">{{data_actual_issue[index]}}</td>
                    <td class="center">{{data_price[index]['price']}}</td>
                    <td class="center">{{data_price[index]['amount']}}</td>
                </tr>
            {% endfor %}
        {% endif %}
        </tbody>
        <tfoot>
            <tr>
                ...
                <td></td>
                <td>{{total_amount_number}}</td>
            </tr>
        </tfoot>
    </table>
    <div class="total">
        Tổng số tiền (Viết bằng chữ): {{ total_amount_string }}
    </div>
    ...
    <div class="signature">
        ...
        <table class="invoice-info-container" style="margin-top: 20px" >
            ...
            <tr class="info__company">
                ...
                <td style="text-align: center;padding: 0 5px; vertical-align: start;">
                    {% if receiver_name %}
                    <div class="signature"></div>
                    <span class="text-bold uppcase" style="line-height: 5.20mm; word-wrap: break-word">{{receiver_name}}</span>
                    {% endif %}
