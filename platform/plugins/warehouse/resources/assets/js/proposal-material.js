...
function materialOptions(selectElement) {
    ...
    if (warehouse != '' || warehouse != 0) {
        $.ajax({
            ...
            success: function (data) {
                ...
                selectElement.empty();
                selectElement.append('<option value="">Chọn nguyên phụ liệu</option>');
                ...
                $.each(data, function (index, material) {
                    ...
                    let option = $('<option value="' + material.id + '" price= "' + material.price + '" quantity="' +
...
function addNewRow(rowNumber) {
    ...
    // Duyệt qua từng phần tử trong allMaterial
...
function getAllProcessHouse() {
    $.ajax({
        ...
        success: function (res) {
            ...
            dropdown.append('<option value="">Chọn nhà gia công</option>');
...
function submitForm(e) {
    ...
    var inputFields = $("form input[name^='input__'], form select[name^='material'], form input[name^='quantity']");
    var isValid = true;
...
$(document).ready(function () {
    ...
    $("#add_row").click(function () {
        var currentRow = $("#addr" + rowNumber);
        var materialValue = currentRow.find("select[name='material[]']").val();
        var codeValue = currentRow.find("input[name='input__code_product_process[]']").val();
        var quantityValue = currentRow.find("input[name='quantity[]']").val();

        if (materialValue === "" || codeValue === "" || quantityValue === "") {
            toastr.clear();
            toastr['warning']('Đã vượt qua số lượng nguyên phụ liệu trong kho ');
            return;
        }
        if (rowNumber < lengthMaterial) {
            rowNumber++;
            addNewRow(rowNumber)
        }
        else {
            toastr.clear();
            toastr['warning']('Danh sách nguyên phụ liệu đã hết ');
            return false;
        }
    ...
    $(document).on('change', '.select_material', function (e) {
        ...
        $('tr:not("#addr0") .select_material').not(currentElement).each(function () {
    ...
    $('#choose-warehouse').on('change', function () {
        ...
        if (warehouse == 0) {
            ...
            materialOptions($('tbody tr:not("#addr0") .select_material'));
    ...
    $("form button[type='submit']").on("click", function (e) {
        ...
        $('#tab_logic tbody tr:not("#addr0")').each(function () {
            const $row = $(this);  // Get the current row
            ...
            if (quantity > quantityStock) {
                ...
                isValid = false;
                toastr.clear();
                toastr['error']('Vui lòng chọn sản phẩm.', { timeOut: 3000 });
                return false;
            }
        });
