...
class MaterialIssueHelper
{
    ...
    public function makeMaterialPDF(MaterialOutConfirm $invoice): PDFHelper|Dompdf
    {
    ...
    public function downloadInvoice(MaterialOutConfirm $invoice): Response
    {
        return $this->makeMaterialPDF($invoice)->download(sprintf('material-%s.pdf', $invoice->proposal_code));
    }

    public function streamInvoice(MaterialOutConfirm $invoice): Response
    {
    ...
    {
        $defaultPath = platform_path('plugins/warehouse/resources/views/material-template-pdf/material-issue.tpl');
    ...
    protected function getDataForInvoiceTemplate(MaterialOutConfirm $invoice): array
    {
        ...
        foreach ($invoice->proposalOutDetail as $key => $value) {
            array_push($dataPrice,
            [
                'price' => number_format($value['material_price']),
                'amount' => number_format($value['material_price'] * $value->actualBatchMaterial->quantity)
            ]
            );

            array_push($dataActualIssue, $value->actualBatchMaterial->quantity);

            $totalAmount += $value['material_price'] * $value->actualBatchMaterial->quantity;
            $totalQtyDoc += $value['material_quantity'];
            $totalStartQty += $value->actualBatchMaterial->quantity;
        }
        ...
        $data = [
            ...
            'proposal_detail' => $invoice->proposalOutDetail,
    ...
    public function getDataForPreview(): MaterialOutConfirm
    {
        $matial = new MaterialOutConfirm([
            'id' => 'INV-1',
