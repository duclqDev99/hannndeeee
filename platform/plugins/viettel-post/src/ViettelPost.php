...
use ViettelPost_Error;
use ViettelPost_Rate;
use ViettelPost_Shipment;
use ViettelPost_Transaction;
...
class ViettelPost
{
    ...
    {
        $this->liveApiToken = setting('shipping_viettel_post_production_key');
        $this->testApiToken = setting('shipping_viettel_post_test_key');
        $this->labelFileType = 'PDF';
        $this->sandbox = setting('shipping_viettel_post_sandbox', 1) == 1;
        ...
        $this->packageTypes = config('plugins.viettel-post.general.package_types', []);
        $this->serviceLevels = config('plugins.viettel-post.general.service_levels', []);

        $this->useCache = setting('shipping_viettel_post_cache_response', 1);

        $this->logging = setting('shipping_viettel_post_logging', 1);

        $this->cache = new Cache(app('cache'), self::class);

        $this->logger = Log::channel('viettel-post');
    }
    ...
    public function validate(): array
    {
        ...
        $apiTokenName = trans('plugins/viettel-post::viettel-post.live_api_token');
        if ($this->sandbox) {
            $apiTokenName = trans('plugins/viettel-post::viettel-post.test_api_token');
        }

        if (! $this->getApiKey()) {
            $errors[] = trans('plugins/viettel-post::viettel-post.token_name_is_required', ['name' => $apiTokenName]);
        } elseif (! $this->validateActiveApiToken()) {
            $errors[] = trans('plugins/viettel-post::viettel-post.token_name_is_invalid', ['name' => $apiTokenName]);
        }
    ...
    public function createShipment(array $params)
    {
        ...
        if (! $response) {
            ...
            try {
                $response = ViettelPost_Shipment::create($params)->__toArray(true);
                if (Arr::get($response, 'status') == 'SUCCESS') {
                    ...
                    if ($rates) {
                        ...
                        if (! $rates) {
                            $ratesResponse = ViettelPost_Shipment::get_shipping_rates([
    ...
    protected function getShipmentResponse($response, array $params): array
    {
        ...
        foreach (Arr::get($response, 'rates', []) as $rate) {
            ...
            if ($days) {
                $description = trans('plugins/viettel-post::viettel-post.estimated_days', ['day' => $days]);
            } else {
    ...
    public function canCreateTransaction(Shipment $shipment): bool
    {
        ...
        if ($order && $order->id && $order->shipping_method->getValue() == VIETTEL_POST_SHIPPING_METHOD_NAME
    ...
    {
        try {
            $transaction = ViettelPost_Transaction::create([
                'rate' => $rateId,
                'async' => false,
            ])->__toArray(true);
    ...
    public function retrieveRate(string $rateId)
    {
        ...
        $response = $this->getCacheValue($cacheKey);
        if (! $response) {
            $response = ViettelPost_Rate::retrieve($rateId)->__toArray(true);
            $this->setCacheValue($cacheKey, $response);
        }
    ...
    public function retrieveShipment(string $shipmentId)
    {
        ...
        if (! $response) {
            try {
                $response = ViettelPost_Shipment::retrieve($shipmentId)->__toArray(true);
                $this->setCacheValue($cacheKey, $response);
            } catch (ViettelPost_Error $ex) {
