...
namespace Botble\ProductQrcode\Supports;
    ...
    protected function afterSheet(AfterSheet $event)
    {
        ...
        for ($row = 1; $row <= $highestRow; $row++) {
            for ($column = 'A'; $column <= $highestColumn; $column++) {
                $delegate->setCellValue($column.$row, null);
            }
        }

        $totalColumns = count($this->collection);
        ...
        $delegate->getColumnDimension('A')->setWidth(50);
        ...
        $delegate
            ->getStyle('1:' . count($this->collection))
            ->getAlignment()
            ->setWrapText(true)
            ...
            foreach ($data as $columnData) {
                $rowIndex = 1;
                foreach ($columnData as $cellValue) {
                    $cellCoordinate = $columnIndex . $rowIndex;
                    $delegate->setCellValue($cellCoordinate, $cellValue);
