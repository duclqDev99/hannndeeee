...
class ProposalHubIssueHelper
{

    public function makeMaterialPDF(ProposalHubIssue $invoice): PDFHelper|Dompdf
    {
    ...
    public function downloadInvoice(ProposalHubIssue $invoice): Response
    {
        return $this->makeMaterialPDF($invoice)->download(sprintf('material-%s.pdf', $invoice->proposal_code));
    }

    public function streamInvoice(ProposalHubIssue $invoice): Response
    {
    ...
    protected function getDataForInvoiceTemplate(ProposalHubIssue $invoice): array
    {
        ...
        foreach ($invoice->ProposalHubIssueDetail as $key => $value) {
            $totalQuantity = 0;
        ...
        if($invoice->is_batch === 1 && $invoice->is_odd === 1){
            ...
            if($invoice->is_batch === 1){
                $totalQtyDoc += $invoice->ProposalHubIssueDetail->count();
            }else {
                foreach ($invoice->ProposalHubIssueDetail as $key => $value){
                    if(array_key_exists($value->product_id,$dataProduct)){
                        $dataProduct[$value->product_id]['quantity'] += $value->quantity;
        ...
        $data = [
            ...
            'proposal_detail' => $invoice->ProposalHubIssueDetail,
            ...
            'warehouse_parent_name' => $invoice->warehouseIssue?->hub?->name ?? '',
            'warehouse_parent_address' => $invoice->warehouseIssue?->hub?->address ?? ''
        ];
        return apply_filters('ecommerce_invoice_variables', $data, $invoice);
    }

    public function getDataForPreview(): ProposalHubIssue
    {
        $matial = new ProposalHubIssue([
            'id' => 'INV-1',
