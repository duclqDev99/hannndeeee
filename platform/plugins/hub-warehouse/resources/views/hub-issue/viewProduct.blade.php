...
@section('content')
    ...
    @php
        ...
        if (isset($actualIssue)) {
            $strCenter = $productIssue?->proposal?->is_warehouse == 6 ? 4 : 6;
        }
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="info-group">
                            ...
                            <strong>
                                @if ($productIssue->proposal->is_warehouse == 6)
                                    Xuất đi giải
                                @else
                                    {{ $productIssue->warehouse->name }} -
                                    {{ $productIssue->warehouse?->hub?->name ?:
                                        ($productIssue->warehouse?->agent?->name ?:
                                            ($productIssue->warehouse?->showroom?->name ?:
                                                ($productIssue->warehouse?->saleWarehouse?->name ?:
                                                    ''))) }}
                                @endif
                            </strong>
                        </div>
                                                                        ...
                                                                        <td class=" vertical-align-t" width="20%"
                                                                            ...
                                                                            <div class="wrap-img">
                                                                                ...
                                                                                <img class="thumb-image thumb-image-cartorderlist"
                                                                                    ...
                                                                                    title="{{ $orderProduct->product($orderProduct->product_id)->first()?->name }}"
                                                                                    src="{{ RvMedia::getImageUrl($orderProduct->product($orderProduct->product_id)->first()?->parentProduct?->first()?->image,'thumb',false,RvMedia::getDefaultImage()) }}"
                                                                                    width="60px"
                                                                                    alt="{{ Str::limit($orderProduct->product($orderProduct->product_id)->first()?->name, 10) }}">
                                                                            </div>
                                                                        </td>
                                                                        <td class="pl5 p-r5 text-center " width = "40%">
                            ...
                            <div class="flexbox-layout-sections">
                                <div class="flexbox-layout-section-primary mt20">
                                    <div class="card ui-layout__item">
                                        <div class="wrapper-content
                                        ">
                                            <span class="card-notify-year" style="background-color: #FEC260;">Phiếu nhập
                                                lại giải</span>
                                            <div class="pd-all-20 p-none-t border-top-title-main"
                                                style="
                                                    max-height: 500px;
                                                    ...
                                                    <div class="product-list">
                                                        @foreach ($productInput as $detail)
                                                            @php
                                                                ...
                                                                $value = array_filter($dataProduct, function (
                                                                    $item,
                                                                ) use ($detail) {
    ...
    @include('plugins/qr-scan::scan-warehouse-issue-in-sales', [
        'warehouse_id' => $productIssue->warehouse_issue_id,
        'warehouse_type' => Warehouse::class,
