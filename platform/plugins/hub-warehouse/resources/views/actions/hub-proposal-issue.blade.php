@php
    ...
    if ($item->status == ProposalProductEnum::PENDING ) {
        if ($item->issuer_id == Auth::user()->id && !$item->proposal_receipt_id) {
            $actionDelete =
                ' <li><a data-bs-toggle="tooltip" data-bs-original-title="Delete"   class="dropdown-item text-danger"
                        href="' .
                route('proposal-hub-issue.destroy', $item->id) .
                '"
        ...
        if (Auth::user()->hasPermission('proposal-hub-issue.approve') || Auth::user()->hasPermission('hub-warehouse.all-permissions')) {
            $actionApprove =
                '
                        <li> <a data-bs-toggle="tooltip" class="dropdown-item text-success" href="' .
                route('proposal-hub-issue.approveProposalProductIssue', $item->id) .
    ...
    if ($actionApprove == '') {
        $actionView =
            '
                    ...
                    href="' .
            route('proposal-hub-issue.view', $item->id) .
    ...
    $editView = '';
    if ($item->status->toValue() !== ProposalProductEnum::APPOROVED && !$item->proposal_receipt_id && $item->issuer_id == Auth::user()->id && $item->status->toValue() !== ProposalProductEnum::REFUSE && $item->status->toValue() !== ProposalProductEnum::CONFIRM) {
        $editView =
            '
                    <li><a data-bs-toggle="tooltip"     class="dropdown-item text-primary" data-bs-original-title="Edit" href="' .
            route('proposal-hub-issue.edit', $item->id) .
