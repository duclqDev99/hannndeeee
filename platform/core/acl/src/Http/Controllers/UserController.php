...
class UserController extends BaseSystemController
{
    ...
    public function getUserProfile(User $user, Request $request)
    {
        ...
        $canChangeProfile = ($currentUser->hasPermission('users.edit') && !$currentUser->isSuperUser() && !$user->isSuperUser()) || $currentUser->getKey() == $user->getKey() || $currentUser->isSuperUser();
    ...
    public function postUpdateProfile(User $user, UpdateProfileRequest $request)
    {
        ...
        $hasRightToUpdate = ($currentUser->hasPermission('users.edit') && !$currentUser->isSuperUser() && !$user->isSuperUser()) ||
