...
use Botble\WidgetCustom\AbstractWidget as AbstractWidgetCustom;
...
class ThemeManagementServiceProvider extends ServiceProvider
{
    ...
    protected function bootWidgets(): void
    {
        ...
        if(is_plugin_active('widget-custom')){
            add_filter('widget_rendered', function (string|null $html, AbstractWidgetCustom $widget) {
                if (! setting('show_theme_guideline_link', false) || ! Auth::guard()->check() || ! Auth::guard()->user()->hasPermission('widgets-custom.index')) {
                    return $html;
                }
                ...
                $editLink = route('widgets-custom.index') . '?widget=' . $widget->getId();
                $link = view('packages/theme::guideline-link', [
                    'html' => $html,
                    'editLink' => $editLink,
                    'editLabel' => __('Edit this widget custom'),
                ])->render();
